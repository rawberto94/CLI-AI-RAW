...
DO $$ BEGIN
  IF NOT EXISTS (
    ...
    WHERE table_name = 'ContractEmbedding' AND column_name = 'embeddingHalf'
  ) THEN
    ALTER TABLE "ContractEmbedding" ADD COLUMN "embeddingHalf" halfvec(1536);
    RAISE NOTICE 'Added embeddingHalf halfvec(1536) column';
  ELSE
    RAISE NOTICE 'embeddingHalf column already exists';
  END IF;
END $$;
...
DO $$ BEGIN
  ...
  WHERE "embeddingHalf" IS NULL AND "embedding" IS NOT NULL;
...
DO $$ BEGIN
  IF NOT EXISTS (
    ...
    IF EXISTS (
      ...
      CREATE INDEX idx_contract_embedding_halfvec_hnsw
    ...
    END IF;
  ELSE
    RAISE NOTICE 'HNSW halfvec index already exists';
  END IF;
END $$;
