...
-- Adds a halfvec(1024) shadow column + HNSW index for ~50% storage savings
-- on the vector index while preserving the original float32 embedding.
--
-- Strategy:
--   1. Add "embeddingHalf" halfvec(1024) column (nullable)
...
ADD COLUMN "embeddingHalf" halfvec(1024);
RAISE NOTICE 'Added embeddingHalf halfvec(1024) column';
...
SET "embeddingHalf" = "embedding"::halfvec(1024)
