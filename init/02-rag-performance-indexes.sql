...
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_indexes WHERE indexname = 'idx_contract_embedding_vector_hnsw'
  ) THEN
    -- Only create if table and column exist
    IF EXISTS (
      SELECT 1 FROM information_schema.columns 
      WHERE table_name = 'ContractEmbedding' AND column_name = 'embedding'
    ) THEN
      -- Ensure the vector column has explicit 1536 dimensions (text-embedding-3-small)
      ALTER TABLE "ContractEmbedding" ALTER COLUMN embedding TYPE vector(1536);
      
      CREATE INDEX idx_contract_embedding_vector_hnsw 
        ON "ContractEmbedding" 
        USING hnsw ("embedding" vector_cosine_ops)
        WITH (m = 16, ef_construction = 64);
      RAISE NOTICE 'Created HNSW vector index on ContractEmbedding.embedding';
    END IF;
  ELSE
    RAISE NOTICE 'HNSW vector index already exists';
  END IF;
END $$;
...
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_indexes WHERE indexname = 'idx_contract_embedding_chunktext_fts'
  ) THEN
    IF EXISTS (
      SELECT 1 FROM information_schema.columns 
      WHERE table_name = 'ContractEmbedding' AND column_name = 'chunkText'
    ) THEN
      CREATE INDEX idx_contract_embedding_chunktext_fts
        ON "ContractEmbedding"
        USING gin (to_tsvector('english', "chunkText"));
      RAISE NOTICE 'Created GIN full-text index on ContractEmbedding.chunkText';
    END IF;
  ELSE
    RAISE NOTICE 'GIN full-text index on chunkText already exists';
  END IF;
END $$;
...
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_indexes WHERE indexname = 'idx_contract_rawtext_fts'
  ) THEN
    IF EXISTS (
      SELECT 1 FROM information_schema.columns 
      WHERE table_name = 'Contract' AND column_name = 'rawText'
    ) THEN
      CREATE INDEX idx_contract_rawtext_fts
        ON "Contract"
        USING gin (to_tsvector('english', "rawText"))
        WHERE "rawText" IS NOT NULL;
      RAISE NOTICE 'Created GIN full-text index on Contract.rawText';
    END IF;
  ELSE
    RAISE NOTICE 'GIN full-text index on rawText already exists';
  END IF;
END $$;
