...
export const errorTemplates: Record<ErrorCode, ErrorTemplate> = {
    ...
    getMessage: (foundHeaders?: string[]) =>
      foundHeaders
        ? `Found headers: ${foundHeaders.join(
            ", "
          )}. These don't match expected rate card columns.`
  ...
  INVALID_RATE: {
    ...
    getMessage: (rows: number[]) =>
      `Found invalid rates in ${rows.length} row${
        rows.length === 1 ? "" : "s"
  ...
  MISSING_REQUIRED_FIELD: {
    ...
    getMessage: (field: string, rows: number[]) =>
      `${field} is missing in ${rows.length} row${
        rows.length === 1 ? "" : "s"
      }: ${rows.slice(0, 5).join(", ")}${rows.length > 5 ? "..." : ""}`,
    suggestions: [
  ...
  UNKNOWN_ERROR: {
    ...
    getMessage: () => "An unexpected error occurred. We've logged the details.",
    suggestions: [
 ...
 */
export function createError(code: ErrorCode, ...args: any[]): ImportError {
