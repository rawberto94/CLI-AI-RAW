/**
 * Direct Database Seeding: Indirect Procurement Taxonomy (Generic)
 * 
 * This script populates the taxonomy management system with a comprehensive
 * indirect procurement category hierarchy using Prisma directly.
 * 
 * Usage: npx tsx scripts/seed-taxonomy-direct.ts
 */
...
async function createCategory(
  ...
  const result = await prisma.taxonomyCategory.upsert({
    where: {
      tenantId_name: {
        tenantId: TENANT_ID,
        name: category.name,
...
async function seedTaxonomy(
  ...
  for (const category of categories) {
    ...
    const id = await createCategory(category, parentId, level, parentPath, sortOrder);
    count++;

    // Recursively create children
    if (category.children && category.children.length > 0) {
      const childPath = parentPath ? `${parentPath}/${category.name}` : `/${category.name}`;
      const childCount = await seedTaxonomy(category.children, id, level + 1, childPath);
      count += childCount;
    }
  }
...
async function main() {
  ...
  try {
    ...
    console.log("");
    console.log("================================================");
    console.log(`✅ Completed! Created/Updated ${totalCreated} categories in ${duration}s`);
    console.log("");
