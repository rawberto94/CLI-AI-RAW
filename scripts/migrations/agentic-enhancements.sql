...
CREATE TABLE IF NOT EXISTS user_feedback_log (
  ...
  feedback_type TEXT NOT NULL CHECK (feedback_type IN ('artifact_edit', 'artifact_regeneration', 'quality_rating', 'error_report', 'positive_feedback')),
...
CREATE INDEX IF NOT EXISTS idx_feedback_tenant_artifact 
  ON user_feedback_log(tenant_id, artifact_type, timestamp DESC);

CREATE INDEX IF NOT EXISTS idx_feedback_type 
  ON user_feedback_log(feedback_type, timestamp DESC);

CREATE INDEX IF NOT EXISTS idx_feedback_user 
  ON user_feedback_log(user_id, timestamp DESC);
...
CREATE TABLE IF NOT EXISTS quality_thresholds (
  ...
  adjustment_magnitude FLOAT CHECK (adjustment_magnitude >= 0 AND adjustment_magnitude <= 0.5),
  updated_at TIMESTAMPTZ NOT NULL DEFAULT NOW(),
  created_at TIMESTAMPTZ NOT NULL DEFAULT NOW(),
  UNIQUE (tenant_id, artifact_type)
);

CREATE INDEX IF NOT EXISTS idx_quality_thresholds_tenant 
  ON quality_thresholds(tenant_id, artifact_type);

CREATE INDEX IF NOT EXISTS idx_quality_thresholds_updated 
  ON quality_thresholds(updated_at DESC);
...
CREATE INDEX IF NOT EXISTS idx_ab_test_name_timestamp 
  ON ab_test_results(test_name, timestamp DESC);

CREATE INDEX IF NOT EXISTS idx_ab_test_tenant 
  ON ab_test_results(tenant_id, timestamp DESC);

CREATE INDEX IF NOT EXISTS idx_ab_test_variant 
  ON ab_test_results(test_name, variant_id);

CREATE INDEX IF NOT EXISTS idx_ab_test_quality 
  ON ab_test_results(test_name, quality_score DESC);

COMMENT ON TABLE ab_test_results IS 'A/B testing results for prompt and model optimization';
COMMENT ON COLUMN ab_test_results.test_name IS 'Name of the A/B test (e.g., "overview-model-test")';
COMMENT ON COLUMN ab_test_results.variant_id IS 'ID of the test variant used (e.g., "gpt-4o-mini")';
COMMENT ON COLUMN ab_test_results.variant_name IS 'Human-readable variant name';
COMMENT ON COLUMN ab_test_results.generation_time IS 'Generation time in milliseconds';
COMMENT ON COLUMN ab_test_results.token_count IS 'Number of tokens used';
COMMENT ON COLUMN ab_test_results.cost IS 'Cost in USD';
COMMENT ON COLUMN ab_test_results.user_accepted IS 'Did user accept without edits?';
COMMENT ON COLUMN ab_test_results.user_rating IS 'User quality rating 1-5';
...
CREATE INDEX IF NOT EXISTS idx_ab_winners_determined 
  ON ab_test_winners(determined_at DESC);
...
CREATE TABLE IF NOT EXISTS agent_performance_log (
  ...
  agent_type TEXT NOT NULL CHECK (agent_type IN ('LEGAL', 'PRICING', 'COMPLIANCE', 'RISK', 'OPERATIONS')),
  artifact_type TEXT NOT NULL,
...
CREATE INDEX IF NOT EXISTS idx_agent_performance_tenant 
  ON agent_performance_log(tenant_id, timestamp DESC);

CREATE INDEX IF NOT EXISTS idx_agent_performance_type 
  ON agent_performance_log(agent_type, timestamp DESC);
...
CREATE INDEX IF NOT EXISTS idx_risk_detection_tenant 
  ON risk_detection_log(tenant_id, detected_at DESC);

CREATE INDEX IF NOT EXISTS idx_risk_detection_contract 
  ON risk_detection_log(contract_id, detected_at DESC);

CREATE INDEX IF NOT EXISTS idx_risk_detection_severity 
  ON risk_detection_log(severity, detected_at DESC);

CREATE INDEX IF NOT EXISTS idx_risk_detection_unresolved 
  ON risk_detection_log(tenant_id, resolved) WHERE resolved = FALSE;
...
CREATE TABLE IF NOT EXISTS intent_detection_log (
  ...
  user_query TEXT,
  user_role TEXT,
  detected_goal TEXT NOT NULL CHECK (detected_goal IN (
    'NEGOTIATE',
    'RISK_ASSESSMENT',
    'COST_OPTIMIZATION',
    'COMPLIANCE_CHECK',
...
CREATE INDEX IF NOT EXISTS idx_intent_detection_tenant 
  ON intent_detection_log(tenant_id, detected_at DESC);

CREATE INDEX IF NOT EXISTS idx_intent_detection_goal 
  ON intent_detection_log(detected_goal, detected_at DESC);

CREATE INDEX IF NOT EXISTS idx_intent_detection_confidence 
  ON intent_detection_log(confidence DESC);
...
GROUP BY test_name, variant_id, variant_name;
...
SELECT
  ...
  AVG(cost) as avg_cost,
  AVG(quality_score) as avg_quality,
  SUM(CASE WHEN won_negotiation = true THEN 1 ELSE 0 END)::float / 
    NULLIF(COUNT(*), 0) as win_rate
FROM agent_performance_log
WHERE timestamp > NOW() - INTERVAL '30 days'
GROUP BY agent_type, artifact_type;
...
SELECT
  ...
  AVG(EXTRACT(EPOCH FROM (acknowledged_at - detected_at)) / 3600) as avg_hours_to_acknowledge,
  AVG(EXTRACT(EPOCH FROM (resolved_at - detected_at)) / 3600) as avg_hours_to_resolve
FROM risk_detection_log
WHERE detected_at > NOW() - INTERVAL '30 days'
GROUP BY tenant_id, risk_type, severity;
...
SELECT
  ...
  SUM(CASE WHEN correct = true THEN 1 ELSE 0 END)::float / 
    NULLIF(SUM(CASE WHEN correct IS NOT NULL THEN 1 ELSE 0 END), 0) as accuracy_rate
...
CREATE OR REPLACE FUNCTION get_quality_thresholds(
  p_tenant_id TEXT,
...
CREATE OR REPLACE FUNCTION get_ab_test_winner(
  p_test_name TEXT
)
RETURNS TABLE (
  variant_id TEXT,
...
DO $$
BEGIN
  RAISE NOTICE 'Agentic AI Enhancements migration completed successfully';
