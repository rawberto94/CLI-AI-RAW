...
CREATE INDEX IF NOT EXISTS "idx_ce_tenant_type" ON "ContractEmbedding" ("tenantId", "contractType");
-- 2. Composite index: (tenantId, contractId) — covers single-contract search
--    within a tenant (most common RAG query pattern)
CREATE INDEX IF NOT EXISTS "idx_ce_tenant_contract" ON "ContractEmbedding" ("tenantId", "contractId");
...
CREATE INDEX IF NOT EXISTS "idx_ce_tenant_chunktype" ON "ContractEmbedding" ("tenantId", "chunkType");
...
CREATE INDEX IF NOT EXISTS "idx_ce_tenant_contract_chunk" ON "ContractEmbedding" ("tenantId", "contractId", "chunkIndex");
...
CREATE INDEX IF NOT EXISTS "idx_ce_chunktext_fts" ON "ContractEmbedding" USING GIN (to_tsvector('english', "chunkText"));
-- 6. Partial index: embedding IS NOT NULL — skip rows without embeddings
--    (e.g., failed embedding generation) during vector search
CREATE INDEX IF NOT EXISTS "idx_ce_has_embedding" ON "ContractEmbedding" ("tenantId", "contractId")
