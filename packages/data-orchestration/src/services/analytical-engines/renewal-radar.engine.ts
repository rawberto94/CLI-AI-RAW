...
export class RenewalRadarEngineImpl implements RenewalRadarEngine {
  ...
  async extractRenewalData(contractId: string): Promise<any> {
    try {
      ...
      try {
        ...
        result.success = true;
        result.confidence = this.calculateExtractionConfidence(
          renewalData,
          contract
        );
        ...
        logger.info(
          { contractId, renewalType: renewalData.renewalType },
          "Renewal data extracted successfully"
        );
        return result;
      } catch (error) {
  ...
  async scheduleAlerts(renewalData: any): Promise<void> {
    try {
      ...
      const daysUntilExpiry = Math.ceil(
        (renewalData.endDate.getTime() - now.getTime()) / (1000 * 60 * 60 * 24)
      );
      ...
      if (
        renewalData.renewalType === "auto" &&
        daysUntilExpiry <= this.alertThresholds.critical
      ) {
        priority = "critical";
      ...
      const alert: RenewalAlert = {
        ...
        recommendations: this.generateRenewalRecommendations(
          renewalData,
          daysUntilExpiry
        ),
      };
      ...
      logger.info(
        { contractId: renewalData.contractId, priority },
        "Renewal alert scheduled"
      );
    } catch (error) {
      logger.error(
        { error, contractId: renewalData.contractId },
    ...
    try {
      ...
      for (const contract of contracts) {
        ...
        if (renewalData.success && renewalData.renewalData) {
          ...
          renewals.push({
            ...
            contractName:
              contract.title || `Contract ${contract.id.substring(0, 8)}`,
      ...
      const calendar: RenewalCalendar = {
        ...
        summary: {
          ...
          totalValue,
          averageDaysToExpiry:
            renewals.length > 0 ? Math.round(totalDays / renewals.length) : 0,
        },
      };
      ...
      logger.info(
        { renewalCount: renewals.length, totalValue },
        "Renewal calendar generated"
      );
      return calendar;
    } catch (error) {
  ...
  async triggerRfxGeneration(contractId: string): Promise<any> {
    try {
      ...
      const rfxStartDate = new Date(
        renewal.endDate.getTime() -
          (renewal.noticePeriod + 30) * 24 * 60 * 60 * 1000
      );
  ...
  private determineRenewalType(contract: any): "manual" | "auto" | "evergreen" {
    ...
    if (
      description.includes("auto-renew") ||
      description.includes("automatically renew")
    ) {
      return "auto";
    }
    if (
      description.includes("evergreen") ||
      description.includes("perpetual")
    ) {
      return "evergreen";
    }
    return "manual";
  ...
  private calculateExtractionConfidence(
    ...
    if (renewalData.autoRenewalClause !== "No auto-renewal clause found")
      confidence += 0.2;
  ...
  private generateRenewalRecommendations(
    ...
    if (
      renewalData.renewalType === "auto" &&
      daysUntilExpiry <= renewalData.noticePeriod
    ) {
      recommendations.push(
        "CRITICAL: Auto-renewal deadline approaching - immediate action required"
      );
    }
    ...
    if (renewalData.riskLevel === "high") {
      recommendations.push(
        "Consider market alternatives due to high risk profile"
      );
  ...
  private async getContractsForRenewal(
  ...
  // Advanced Renewal Analytics Methods
  private async getHistoricalRenewalData(tenantId: string): Promise<any[]> {
    ...
    const historicalContracts = await dbAdaptor.prisma.contract.findMany({
      where: {
        tenantId,
        status: { in: ["COMPLETED" as const, "ARCHIVED" as const] },
      },
  ...
  private async buildRenewalPredictionModel(
    historicalData: any[]
  ): Promise<any> {
  ...
  private async generateRenewalForecasts(
    model: any,
    timeframe: number
  ): Promise<any[]> {
  ...
  private async generateValueForecasts(
    model: any,
    timeframe: number
  ): Promise<any[]> {
  ...
  private async generateRiskForecasts(
    model: any,
    timeframe: number
  ): Promise<any[]> {
  ...
  private generatePredictiveInsights(predictions: any[]): any[] {
    ...
    const highValueRenewals = predictions.filter(
      (p) => p.predictedValue > 1000000
    );
    ...
    // Seasonal patterns
    const q4Renewals = predictions.filter(
      (p) => p.month >= 10 && p.month <= 12
    );
  ...
  private calculatePredictionConfidence(
    ...
    return {
      overall:
        (renewalModel.accuracy + valueModel.accuracy + riskModel.accuracy) / 3,
  ...
  private generatePredictiveRecommendations(
    ...
    if (highRiskCount > 0) {
      recommendations.push(
        `Develop mitigation strategies for ${highRiskCount} high-risk renewals`
      );
    }
    ...
    if (totalValue > 10000000) {
      recommendations.push(
        "Allocate dedicated renewal management resources for high-value portfolio"
      );
    }
    ...
    for (const insight of insights) {
      if (insight.type === "seasonal_concentration") {
        recommendations.push(
          "Plan early engagement for Q4 renewal concentration"
        );
      }
    }

    recommendations.push(
      "Implement continuous monitoring for prediction accuracy"
    );
  ...
  private calculateScenarioScore(scenario: any): number {
    ...
    const effortScore =
      scenario.effort === "low" ? 1 : scenario.effort === "medium" ? 0.7 : 0.4;
  ...
  private async storeOptimizationPlan(result: any): Promise<void> {
    try {
      ...
      logger.error(
        ...
        "Failed to store optimization plan"
      );
    }
  }
  ...
  private determineRenewalOutcome(
    contract: any
  ...
  private extractRiskFactors(contract: any): string[] {
    ...
    if (contract.supplierName?.includes("offshore"))
      factors.push("geographic_risk");
