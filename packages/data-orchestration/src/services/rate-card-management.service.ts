...
export class RateCardManagementService {
  ...
  async createRateCard(
    rateCard: ManualRateCard,
    tenantId: string = "default"
  ): Promise<{ success: boolean; data?: any; error?: any }> {
    try {
      logger.info(
        { supplierId: rateCard.supplierId, tenantId },
        "Creating manual rate card"
      );

      // Standardize supplier name
      const supplierStandardization =
        await dataStandardizationService.standardizeSupplier(
          rateCard.supplierName
        );
      ...
      for (const rate of rateCard.rates) {
        ...
        const seniorityStandardization =
          await dataStandardizationService.standardizeSeniority(rate.level, {
            role: rate.role,
          });
      ...
      logger.info(
        { rateCardId, rateCount: createdRates.length },
        "Manual rate card created successfully"
      );
  ...
  async updateRateCard(
    rateCardId: string,
    updates: Partial<ManualRateCard>,
    tenantId: string = "default"
  ): Promise<{ success: boolean; data?: any; error?: any }> {
    try {
      ...
      if (
        ...
        updates.currency ||
        updates.region ||
        updates.deliveryModel
      ) {
        // This would update the rate_cards table
        // For now, we'll log the update
        logger.info(
          { rateCardId, updates },
          "Rate card metadata update requested"
        );
      }
      ...
      if (updates.rates) {
        ...
        logger.info(
          { rateCardId, updatedRates: updatedRates.length },
          "Rate card rates updated"
        );
      }
  ...
  async processBulkUpload(
    ...
    try {
      logger.info(
        { rowCount: data.length, tenantId },
        "Processing bulk rate card upload"
      );
      ...
      for (let i = 0; i < data.length; i++) {
        ...
        try {
          ...
          if (parsedRow.effectiveDate < minDate)
            minDate = parsedRow.effectiveDate;
          if (parsedRow.effectiveDate > maxDate)
            maxDate = parsedRow.effectiveDate;

          // Group by supplier + effective date
          const key = `${parsedRow.supplierId}_${
            parsedRow.effectiveDate.toISOString().split("T")[0]
          }`;
      ...
      for (const [key, rateCard] of rateCardMap) {
        try {
          ...
          result.errors.push({
            row: 0,
            error: `Rate card creation error: ${
              error instanceof Error ? error.message : "Unknown error"
            }`,
            data: rateCard,
          });
  ...
  private validateBulkRow(
    ...
    if (!row.hourly_rate && !row.daily_rate && !row.monthly_rate) {
      return {
        valid: false,
        error: "At least one rate (hourly/daily/monthly) is required",
      };
    }
    ...
    if (
      row.delivery_model &&
      !["onshore", "nearshore", "offshore"].includes(row.delivery_model)
    ) {
      return {
        valid: false,
        error:
          "Invalid delivery model. Must be: onshore, nearshore, or offshore",
      };
    }
  ...
  private parseBulkRow(row: any): any {
    return {
      ...
      deliveryModel: (row.delivery_model || "onshore") as
        | "onshore"
        | "nearshore"
        | "offshore",
  ...
  async healthCheck(): Promise<boolean> {
    try {
      ...
      logger.error(
        { error },
        "Rate card management service health check failed"
      );
      return false;
    }
  }
...
export const rateCardManagementService =
  RateCardManagementService.getInstance();
