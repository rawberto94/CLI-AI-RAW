...
exports.ragIntegrationService = void 0;
const clients_db_1 = __importDefault(require("clients-db"));
const logger_1 = require("../utils/logger");
const logger = (0, logger_1.createLogger)('rag-integration-service');
class RagIntegrationService {
     ...
     */
    async reindexContract(contractId) {
        const prisma = (0, clients_db_1.default)();
        try {
            ...
            const metadataSummary = this.buildMetadataSummary(contract.metadata);
            ...
            const { toSql } = await Promise.resolve().then(() => __importStar(require('pgvector/utils')));
    ...
    buildArtifactSummary(artifacts) {
        if (!artifacts || artifacts.length === 0)
            return '';
        const lines = ['EXTRACTED ARTIFACTS:'];
        for (const artifact of artifacts) {
            ...
            switch (artifact.artifactType) {
                case 'contract_metadata':
                    if (data.partyA)
                        lines.push(`Party A: ${data.partyA}`);
                    if (data.partyB)
                        lines.push(`Party B (Supplier): ${data.partyB}`);
                    if (data.effectiveDate)
                        lines.push(`Effective Date: ${data.effectiveDate}`);
                    if (data.expirationDate)
                        lines.push(`Expiration Date: ${data.expirationDate}`);
                    if (data.totalValue)
                        lines.push(`Contract Value: $${Number(data.totalValue).toLocaleString()}`);
                    if (data.contractType)
                        lines.push(`Contract Type: ${data.contractType}`);
                ...
                case 'OVERVIEW':
                    ...
                    if (data.summary?.value)
                        lines.push(`Summary: ${data.summary.value}`);
                    ...
                    if (data.contractType?.value)
                        lines.push(`Contract Type: ${data.contractType.value}`);
                    if (data.effectiveDate?.value)
                        lines.push(`Effective Date: ${data.effectiveDate.value}`);
                    if (data.expirationDate?.value)
                        lines.push(`Expiration Date: ${data.expirationDate.value}`);
                    if (data.term?.value)
                        lines.push(`Term: ${data.term.value}`);
                    if (data.jurisdiction?.value)
                        lines.push(`Jurisdiction: ${data.jurisdiction.value}`);
                ...
                case 'CLAUSES':
                    ...
                    if (Array.isArray(data.clauses)) {
                        for (const clause of data.clauses.slice(0, 15)) {
                            ...
                            if (clause.content)
                                lines.push(`  ${clause.content.slice(0, 200)}`);
                        ...
                        for (const table of data.rawRateTables) {
                            ...
                            if (Array.isArray(table.rows)) {
                                for (const row of table.rows.slice(0, 15)) {
                                    const values = table.headers?.map((h) => row[h] || '').join(' | ');
                                    lines.push(`    ${values}`);
                                }
                            }
                            if (table.notes)
                ...
                case 'RISK':
                    ...
                    if (Array.isArray(data.riskFactors)) {
                        ...
                        for (const risk of data.riskFactors.slice(0, 10)) {
                            ...
                            if (risk.affectedParty)
                                lines.push(`    Affects: ${risk.affectedParty}`);
                        }
                    }
                ...
                case 'FINANCIAL':
                    ...
                    if (Array.isArray(data.offers)) {
                        for (const offer of data.offers) {
                            ...
                            if (offer.validityPeriod)
                                lines.push(`  Valid for: ${offer.validityPeriod}`);
                            if (offer.totalAmount)
                                lines.push(`  Total: $${Number(offer.totalAmount).toLocaleString()}`);
                ...
                case 'OBLIGATIONS':
                    ...
                    if (Array.isArray(data.obligations)) {
                        ...
                        for (const ob of data.obligations.slice(0, 20)) {
                            ...
                            if (ob.status)
                                lines.push(`    Status: ${ob.status}`);
                            if (ob.priority)
                                lines.push(`    Priority: ${ob.priority}`);
                ...
                case 'RENEWAL':
                    ...
                    if (data.noticeRequirements) {
                        ...
                        if (nr.noticePeriod)
                            lines.push(`  Notice Period: ${nr.noticePeriod}`);
                        if (nr.noticeMethod)
                            lines.push(`  Method: ${nr.noticeMethod}`);
                        if (nr.noticeRecipient)
                            lines.push(`  Recipient: ${nr.noticeRecipient}`);
                    }
                    if (data.terminationRights) {
                        const tr = data.terminationRights;
                        lines.push('Termination Rights:');
                        if (tr.forCause)
                            lines.push(`  For Cause: ${tr.forCause}`);
                        if (tr.forConvenience)
                            lines.push(`  For Convenience: ${tr.forConvenience}`);
                        if (tr.noticePeriod)
                            lines.push(`  Notice Period: ${tr.noticePeriod}`);
                    }
                ...
                case 'NEGOTIATION':
                    ...
                    if (Array.isArray(data.negotiationPoints)) {
                        ...
                        for (const np of data.negotiationPoints.slice(0, 10)) {
                            ...
                            if (np.currentTerms)
                                lines.push(`    Current: ${np.currentTerms}`);
                            if (np.suggestedChange)
                                lines.push(`    Suggested: ${np.suggestedChange}`);
                            if (np.impact)
                                lines.push(`    Impact: ${np.impact}`);
                        }
                    }
                ...
                case 'AMENDMENTS':
                    ...
                    if (Array.isArray(data.amendments)) {
                        ...
                        for (const am of data.amendments.slice(0, 10)) {
                            ...
                            if (am.date || am.effectiveDate)
                                lines.push(`    Date: ${am.date || am.effectiveDate}`);
                    ...
                    if (Array.isArray(data.changes)) {
                        ...
                        for (const ch of data.changes.slice(0, 15)) {
                            ...
                            if (ch.newText)
                                lines.push(`    New: ${ch.newText.slice(0, 100)}`);
                ...
                case 'CONTACTS':
                    ...
                    if (Array.isArray(contactList) && contactList.length > 0) {
                        ...
                        for (const contact of contactList.slice(0, 15)) {
                            ...
                            if (contact.isSignatory)
                                lines.push(`    Signatory: Yes`);
                            if (contact.isPrimaryContact)
                                lines.push(`    Primary Contact: Yes`);
                    ...
                    if (Array.isArray(data.signatories)) {
                        ...
                        for (const sig of data.signatories.slice(0, 10)) {
                            ...
                            if (sig.dateSigned)
                                lines.push(`    Signed: ${sig.dateSigned}`);
                        }
                    }
                    ...
                    if (Array.isArray(noticeAddrs) && noticeAddrs.length > 0) {
                        ...
                        for (const addr of noticeAddrs) {
                            ...
                            if (addr.attention)
                                lines.push(`    Attn: ${addr.attention}`);
                        }
                    }
    ...
    buildMetadataSummary(metadata) {
        if (!metadata)
            return '';
        const lines = ['CONTRACT METADATA:'];
        if (metadata.contractTitle)
            lines.push(`Title: ${metadata.contractTitle}`);
        if (metadata.supplierName)
            lines.push(`Supplier: ${metadata.supplierName}`);
        if (metadata.category)
            lines.push(`Category: ${metadata.category}`);
        if (metadata.subcategory)
            lines.push(`Subcategory: ${metadata.subcategory}`);
        if (metadata.department)
            lines.push(`Department: ${metadata.department}`);
        if (metadata.costCenter)
            lines.push(`Cost Center: ${metadata.costCenter}`);
...
exports.ragIntegrationService = RagIntegrationService.getInstance();
