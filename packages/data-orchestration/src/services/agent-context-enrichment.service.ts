...
import { neo4jGraphService } from './graph/neo4j-graph.service';
...
export class AgentContextEnrichmentService {
  ...
  private async fetchGraphInsights(
    ...
    try {
      // Neo4j initializes in constructor, just verify connection
      // If not connected, queries will fail gracefully
      
      // Get related contracts through graph
      const relatedContracts = await neo4jGraphService.findRelatedContracts(
        contractId,
        tenantId,
        { maxDepth: 2, minStrength: 0.3 }
      );
      
      // Get contract cluster
      const cluster = await neo4jGraphService.getContractCluster(contractId, tenantId);
      
      // Get key entities for this contract
      const entities = await neo4jGraphService.getContractEntities(contractId, tenantId);
      
      // Calculate importance score (PageRank-style)
      const importanceScore = await neo4jGraphService.calculateImportanceScore(
        contractId,
        tenantId
      );
      
      return {
        relatedContracts: relatedContracts.map(r => ({
          contractId: r.contractId,
          relationship: r.relationshipType,
          strength: r.strength,
          commonParties: r.commonParties || [],
        })),
        ...
        counterpartyNetwork: cluster?.counterpartyAnalysis,
        importanceScore,
      };
    } catch (error) {
      logger.warn({ error, contractId }, 'Neo4j graph insights failed, returning empty');
      return {
        relatedContracts: [],
        keyEntities: [],
        importanceScore: 0.5,
