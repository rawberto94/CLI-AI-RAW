...
export class EnhancedRateAnalyticsService {
  ...
  static getInstance(): EnhancedRateAnalyticsService {
    if (!EnhancedRateAnalyticsService.instance) {
      EnhancedRateAnalyticsService.instance =
        new EnhancedRateAnalyticsService();
    }
  ...
  async analyzeByLineOfService(
    tenantId: string,
    filters?: EnhancedRateCardFilters
  ): Promise<LineOfServiceAnalytics> {
    try {
      ...
      if (cached && typeof cached === "string") {
        ...
        return cached as LineOfServiceAnalytics;
      }
      ...
      const serviceBreakdown = (await dbAdaptor.prisma.$queryRawUnsafe(
        serviceQuery,
        ...params
      )) as any[];
      ...
      const enhancedBreakdown = serviceBreakdown.map((service: any) => ({
        ...
        marketPosition: this.calculateMarketPosition(
          service.average_rate || 0,
          serviceBreakdown
        ),
        trendDirection: this.calculateTrendDirection(
          service.average_rate || 0,
          service.rate_variance || 0
        ) as TrendDirection,
        topRoles: service.top_roles
          ? service.top_roles.split(",").slice(0, 5)
          : [],
      }));

      // Generate cross-service comparisons
      const crossServiceComparison =
        this.generateServiceComparisons(enhancedBreakdown);

      // Generate recommendations
      const recommendations =
        this.generateLineOfServiceRecommendations(enhancedBreakdown);
      ...
      const analytics: LineOfServiceAnalytics = {
        ...
        avgRateAcrossServices:
          enhancedBreakdown.reduce((sum, s) => sum + s.averageRate, 0) /
          enhancedBreakdown.length,
      };
      ...
      logger.error(
        ...
        "Failed to analyze by line of service"
      );
      throw error;
    }
  }
  ...
  async analyzeBySeniority(
    tenantId: string,
    filters?: EnhancedRateFilters
  ): Promise<SeniorityAnalytics> {
    try {
      logger.info({ tenantId, filters }, "Analyzing rates by seniority");

      const cacheKey = `seniority-analytics:${tenantId}:${JSON.stringify(
        filters
      )}`;
      const cached = await cacheAdaptor.get(cacheKey);
      if (cached && typeof cached === "string") {
        ...
        return cached as SeniorityAnalytics;
      }
      ...
      const seniorityData = (await dbAdaptor.prisma.$queryRawUnsafe(
        seniorityQuery,
        ...params
      )) as any[];

      // Calculate progression gaps and market benchmarks
      const seniorityProgression = await Promise.all(
        seniorityData.map(async (level: any, index: number) => {
          const nextLevel = seniorityData[index + 1];
          const progressionGap = nextLevel
            ? nextLevel.average_rate - level.average_rate
            : undefined;
          ...
          return {
            ...
            marketBenchmark: await this.getMarketBenchmark(
      ...
      const careerPathAnalysis =
        this.generateCareerPathAnalysis(seniorityProgression);

      // Generate gap analysis
      const gapAnalysis =
        this.generateSeniorityGapAnalysis(seniorityProgression);
      ...
      const analytics: SeniorityAnalytics = {
        ...
        avgProgressionIncrease:
          this.calculateAverageProgressionIncrease(seniorityProgression),
      };
      ...
      logger.error(
        { error, tenantId, filters },
        "Failed to analyze by seniority"
      );
      throw error;
    }
  }
  ...
  async analyzeByGeography(
    ...
    try {
      ...
      if (cached && typeof cached === "string") {
        ...
        return cached as GeographicAnalytics;
      }
      ...
      const geoData = (await dbAdaptor.prisma.$queryRawUnsafe(
        geoQuery,
        ...params
      )) as any[];
      ...
      const locationBreakdown = await Promise.all(
        geoData.map(async (geo: any) => {
          ...
          const adjustedRate =
            await rateCalculationEngine.applyGeographicAdjustment(
              geo.average_rate,
              location
            );
          const marketCompetitiveness = this.calculateMarketCompetitiveness(
            geo.average_rate,
            geoData
          );
          const costAdvantage = this.calculateCostAdvantage(
            geo.average_rate,
            geo.cost_of_living_index || 100
          );
      ...
      const arbitrageOpportunities =
        this.identifyArbitrageOpportunities(locationBreakdown);
      ...
      const analytics: GeographicAnalytics = {
        ...
        avgCostOfLiving:
          locationBreakdown.reduce(
            (sum, loc) => sum + (loc.location.costOfLivingIndex || 100),
            0
      ...
      logger.error(
        { error, tenantId, filters },
        "Failed to analyze by geography"
      );
      throw error;
    }
  }
  ...
  async analyzeSkillPremiums(
    ...
    try {
      ...
      if (cached && typeof cached === "string") {
        ...
        return cached as SkillPremiumAnalytics;
      }
      ...
      const certificationValue = await this.processCertificationBreakdown(
        skillData
      );
      ...
      logger.error(
        { error, tenantId, filters },
        "Failed to analyze skill premiums"
      );
      throw error;
    }
  }
  ...
  async generateMarketBenchmarks(
    ...
    try {
      logger.info(
        { role, seniority, location, tenantId },
      ...
      if (rates.length === 0) {
        throw new Error(
          `No benchmark data available for ${role} ${seniority} in ${location.country}`
        );
      }

      const rateValues = rates
        .map((r: any) => r.hourly_rate)
    ...
    for (let i = 0; i < services.length - 1; i++) {
      for (let j = i + 1; j < services.length; j++) {
        ...
        const percentageDifference =
          (rateDifference / serviceB.averageRate) * 100;
  ...
  private generateLineOfServiceRecommendations(services: any[]): string[] {
    ...
    if (services.length === 0) {
      recommendations.push(
        "No line of service data available. Consider categorizing rate cards by service type."
      );
    ...
    if (variance > 100) {
      recommendations.push(
        "Significant rate variance across services. Consider standardizing rates within service categories."
      );
    }
    ...
    if (lowVolumeServices.length > 0) {
      recommendations.push(
        `${lowVolumeServices.length} services have limited rate data. Consider consolidating or expanding rate coverage.`
      );
    }
    ...
    for (let i = 0; i < progression.length - 1; i++) {
      ...
      if (current.averageRate && next.averageRate) {
        ...
        paths.push({
          ...
          percentageIncrease,
          typicalTimeframe: this.estimateProgressionTimeframe(
            current.level,
            next.level
          ),
        });
  ...
  private generateSeniorityGapAnalysis(progression: any[]): SeniorityGap[] {
    ...
    progression.forEach((level) => {
      const expectedRate =
        industryBenchmarks[level.level as SeniorityLevel] || level.averageRate;
  ...
  private calculateAverageProgressionIncrease(progression: any[]): number {
    ...
    for (let i = 0; i < progression.length - 1; i++) {
      if (progression[i].averageRate && progression[i + 1].averageRate) {
        increases.push(
          progression[i + 1].averageRate - progression[i].averageRate
        );
      }
    }
  ...
  private calculateMarketCompetitiveness(
    ...
    const avgRate =
      allRates.reduce((sum, r) => sum + (r.average_rate || 0), 0) /
      allRates.length;
  ...
  private identifyArbitrageOpportunities(
    ...
    const sortedByRate = [...locationBreakdown].sort(
      ...
      if (rate.required_skills) {
        try {
          ...
          logger.warn(
            ...
            "Failed to parse required skills"
          );
        }
      }
    });
  ...
  private async processCertificationBreakdown(
    skillData: any[]
  ): Promise<any[]> {
    ...
    skillData.forEach((rate) => {
      if (rate.required_certifications) {
        try {
          ...
          logger.warn(
            { error, rateId: rate.id },
            "Failed to parse required certifications"
          );
        }
      }
    });
  ...
  private generateSkillRecommendations(
    ...
    const highValueSkills = skills
      .filter((s) => s.averagePremium > 20)
      .slice(0, 3);
    if (highValueSkills.length > 0) {
      recommendations.push(
        `High-value skills identified: ${highValueSkills
          .map((s) => s.skill)
          .join(", ")}`
      );
    }
    ...
    if (criticalSkills.length > 0) {
      recommendations.push(
        `Critical skills in high demand: ${criticalSkills
          .map((s) => s.skill)
          .join(", ")}`
      );
    }
  ...
  async healthCheck(): Promise<boolean> {
    try {
      ...
      logger.error(
        { error },
        "Enhanced rate analytics service health check failed"
      );
      return false;
    }
  }
...
export const enhancedRateAnalyticsService =
  EnhancedRateAnalyticsService.getInstance();
