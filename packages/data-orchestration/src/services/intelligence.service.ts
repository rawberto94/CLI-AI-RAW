...
export class IntelligenceService {
   ...
   */
  async analyzeContractPatterns(
    tenantId: string
  ): Promise<ServiceResponse<IntelligencePattern[]>> {
    try {
      ...
      logger.info(
        { tenantId, patternCount: patterns.length },
        "Patterns detected"
      );
  ...
  async generateInsights(
    tenantId: string
  ): Promise<ServiceResponse<IntelligenceInsight[]>> {
    try {
      ...
      const processInsights = await this.generateProcessOptimizationInsights(
        patterns,
        contracts
      );
      ...
      const supplierInsights = await this.generateSupplierInsights(
        patterns,
        contracts
      );
  ...
  async getPortfolioAnalytics(
    ...
    try {
      ...
      const analytics: PortfolioAnalytics = {
        ...
        trends: (await this.calculateTrends(contracts)) as any,
        risks: (await this.calculateRisks(contracts)) as any,
  ...
  private async detectFinancialPatterns(
    contracts: any[]
        ...
        patterns.push({
          ...
          metadata: {
            ...
            evidence: outliers.map((o) => ({
              id: o.id,
              value: o.totalValue,
              deviation: Math.abs(Number(o.totalValue) - mean) / stdDev,
            })),
            recommendations: [
    ...
    (Object.entries(currencyGroups) as [string, any[]][]).forEach(
      ([currency, currencyContracts]) => {
        const currencyValue = currencyContracts.reduce(
          (sum: number, c: any) => sum + (Number(c.totalValue) || 0),
          0
        );
        const concentration = currencyValue / totalValue;

        if (concentration > 0.7 && currency !== "USD") {
          // >70% in non-USD
          patterns.push({
            id: `currency-concentration-${currency}-${Date.now()}`,
            type: "financial",
            pattern: "currency_risk",
            description: `High concentration (${(concentration * 100).toFixed(
              1
            )}%) in ${currency}`,
            confidence: 0.9,
            impact: "medium",
            affectedContracts: currencyContracts.map((c: any) => c.id),
            metadata: {
              ...
              evidence: [
                {
                  ...
                  concentration,
                  value: currencyValue,
                  contractCount: currencyContracts.length,
                },
              ],
              recommendations: [
  ...
  private async detectSupplierRiskPatterns(
    contracts: any[]
  ): Promise<IntelligencePattern[]> {
    const patterns: IntelligencePattern[] = [];
    ...
    (Object.entries(supplierGroups) as [string, any[]][]).forEach(
      ([supplier, supplierContracts]) => {
        ...
        if (concentration > 0.3) {
          // >30% concentration
          patterns.push({
            id: `supplier-concentration-${supplier.replace(
              /\s+/g,
              "-"
            )}-${Date.now()}`,
            type: "risk",
            pattern: "supplier_concentration",
            description: `High dependency on ${supplier} (${(
              concentration * 100
            ).toFixed(1)}% of portfolio value)`,
            confidence: 0.95,
            impact:
              concentration > 0.5
                ? "high"
                ...
                ? "high"
                : "medium",
            affectedContracts: supplierContracts.map((c: any) => c.id),
            detectedAt: new Date(),
            metadata: {
              ...
              evidence: [
                {
                  ...
                  averageContractValue:
                    supplierValue / supplierContracts.length,
                },
              ],
              recommendations: [
    ...
    const supplierPerformance = Object.entries(supplierGroups)
      .map(([supplier, contracts]: [string, any]) => {
        const supplierContractsArray = contracts as any[];
        const completedContracts = supplierContractsArray.filter(
          (c: any) => c.status === "COMPLETED"
        );
        const failedContracts = supplierContractsArray.filter(
          (c: any) => c.status === "FAILED"
        );
        const successRate =
          supplierContractsArray.length > 0
            ? completedContracts.length / supplierContractsArray.length
            : 0;

        return {
          supplier,
          contracts: supplierContractsArray,
          successRate,
    ...
    const poorPerformers = supplierPerformance.filter(
      (s) => s.successRate < 0.8 && s.failedCount > 1
    );
    ...
    if (poorPerformers.length > 0) {
      patterns.push({
        ...
        type: "performance",
        pattern: "supplier_performance_issues",
        description: `${poorPerformers.length} suppliers with processing issues detected`,
        confidence: 0.8,
        impact: "medium",
        affectedContracts: poorPerformers.flatMap((s) =>
          s.contracts.map((c) => c.id)
        ),
        metadata: {
  ...
  private async detectTemporalPatterns(
    ...
    const criticalExpirations = expiringContracts.filter(
      (c) => new Date(c.endDate) <= next30Days
    );

    if (expiringContracts.length > 0) {
      const totalExpiringValue = expiringContracts.reduce(
        (sum, c) => sum + (Number(c.totalValue) || 0),
        0
      );
  ...
  private async detectCompliancePatterns(
    contracts: any[]
  ): Promise<IntelligencePattern[]> {
    const patterns: IntelligencePattern[] = [];
    ...
    if (incompleteContracts.length > contracts.length * 0.2) {
      // >20% incomplete
      const missingFieldStats = criticalFields
        .map((field) => ({
          field,
          missingCount: contracts.filter((c) => !c[field]).length,
          percentage:
            (contracts.filter((c) => !c[field]).length / contracts.length) *
            100,
        }))
  ...
  private async detectPerformancePatterns(
    ...
    const processedContracts = contracts.filter(
      (c) => c.uploadedAt && c.processedAt && c.status === "COMPLETED"
    );

    if (processedContracts.length > 5) {
      const processingTimes = processedContracts.map(
        (c) =>
      ...
      const slowContracts = processedContracts.filter(
        (c, i) => processingTimes[i] > avgProcessingTime * 2
      );
  ...
  private async generateCostOptimizationInsights(
    ...
    for (const pattern of financialPatterns) {
      if (pattern.pattern === "value_anomaly") {
        ...
        insights.push({
          ...
          recommendation:
            "Review pricing strategies and negotiate better terms for high-value outlier contracts",
          priority: 2,
          confidence: 0.7,
          impact: "medium",
          potentialSavings,
  ...
  private async generateRiskMitigationInsights(
    ...
    const supplierRiskPatterns = patterns.filter(
      (p) => p.type === "risk" && p.pattern.includes("supplier_concentration")
    );
    ...
    for (const pattern of supplierRiskPatterns) {
      ...
      insights.push({
        ...
        recommendation:
          "Diversify supplier base and develop alternative suppliers to reduce concentration risk",
        priority: pattern.impact === "high" ? 1 : 2,
        confidence: 0.9,
  ...
  private async generateProcessOptimizationInsights(
    ...
    for (const pattern of performancePatterns) {
      if (pattern.pattern === "processing_efficiency") {
        insights.push({
          ...
          description: pattern.description,
          recommendation:
            "Optimize processing pipeline and investigate bottlenecks causing slow processing",
          priority: 3,
          confidence: 0.8,
          impact: "medium",
          generatedAt: new Date(),
  ...
  private async generateComplianceInsights(
    ...
    for (const pattern of compliancePatterns) {
      if (pattern.pattern === "data_completeness") {
        insights.push({
          ...
          description: pattern.description,
          recommendation:
            "Implement mandatory field validation and improve contract intake processes",
          priority: 2,
          confidence: 0.95,
          impact: "medium",
          generatedAt: new Date(),
  ...
  private async generateSupplierInsights(
    ...
    const supplierPerformancePatterns = patterns.filter(
      (p) =>
        p.type === "performance" && p.pattern.includes("supplier_performance")
    );
    ...
    for (const pattern of supplierPerformancePatterns) {
      insights.push({
        ...
        description: pattern.description,
        recommendation:
          "Implement supplier training programs and performance monitoring",
        priority: 3,
        confidence: 0.8,
        impact: "medium",
        generatedAt: new Date(),
  ...
  private calculateOverviewMetrics(contracts: any[]) {
    ...
    return {
      ...
      averageValue:
        contracts.length > 0
          ? contracts.reduce((sum, c) => sum + (Number(c.totalValue) || 0), 0) /
            contracts.length
          : 0,
  ...
  private async calculateTrends(contracts: any[]) {
    ...
    const last30Days = contracts.filter(
      (c) =>
  ...
  private async calculateRisks(contracts: any[]) {
    ...
    const supplierConcentration = Object.entries(supplierGroups)
      .map(([supplier, supplierContracts]) => {
        const supplierValue = (supplierContracts as any[]).reduce(
          (sum, c) => sum + (Number(c.totalValue) || 0),
          0
        );
        const concentration = supplierValue / totalValue;
   ...
   */
  async runIntelligenceAnalysis(tenantId: string): Promise<
    ServiceResponse<{
      patterns: IntelligencePattern[];
      insights: IntelligenceInsight[];
  ...
  > {
    try {
      const [patternsResult, insightsResult, analyticsResult] =
        await Promise.all([
      ...
      return {
        ...
        data: {
          ...
          analytics: analyticsResult.data || ({} as PortfolioAnalytics),
        },
      };
    } catch (error) {
