...
export class DocumentExpiryService {
  static async create(tenantId: string, data: any) {
    const result = await prisma.$queryRawUnsafe(
      ...
      tenantId, data.vendorName, data.vendorId || null, data.documentType,
      data.documentName, data.fileUrl || null, data.issueDate || null,
      data.expiryDate, JSON.stringify(data.alertDaysBefore || [90, 60, 30, 14]),
      data.autoNotify ?? true, JSON.stringify(data.notifyEmails || []), data.createdBy
    );
  ...
  static async list(tenantId: string, filters: { status?: string; vendorName?: string } = {}) {
    ...
    if (filters.vendorName) { where += ` AND vendor_name ILIKE $${idx}`; params.push(`%${filters.vendorName}%`); idx++; }
    return prisma.$queryRawUnsafe(
      `SELECT *, CASE WHEN expiry_date < NOW() THEN 'EXPIRED' WHEN expiry_date < NOW() + INTERVAL '30 days' THEN 'EXPIRING_SOON' ELSE status END as computed_status FROM document_expiry_items ${where} ORDER BY expiry_date ASC`, ...params
    );
  }

  static async getExpiringSoon(tenantId: string, days: number = 30) {
    return prisma.$queryRawUnsafe(
      `SELECT * FROM document_expiry_items WHERE tenant_id = $1 AND status = 'ACTIVE' AND expiry_date BETWEEN NOW() AND NOW() + $2 * INTERVAL '1 day' ORDER BY expiry_date ASC`,
      tenantId, days
    );
  }
