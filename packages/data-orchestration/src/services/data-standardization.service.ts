...
export class DataStandardizationService {
  ...
  private seniorityMappings: Map<string, string> = new Map();
   ...
   */
  async standardizeLineOfService(
    value: string
    ...
    try {
      ...
      const fuzzyMatch = this.findFuzzyMatch(
        normalized,
        Array.from(this.lineOfServiceMappings.keys())
      );
      ...
      const dbResult = await this.findDatabaseMapping(
        "line_of_service",
        normalized
      );
      if (dbResult) {
   ...
   */
  async standardizeSeniority(
    value: string,
    context?: { role?: string; experience?: number }
  ): Promise<StandardizationResult> {
    try {
      const normalized = this.normalizeString(value);
  ...
  async clusterLineOfService(
    ...
    try {
      ...
      const rawData = (await dbAdaptor.prisma.$queryRawUnsafe(
        query,
        tenantId
      )) as any[];
      ...
      const analysis: ClusteringAnalysis = {
        ...
        clusteredItems: clusters.reduce(
      ...
      logger.info(
        { tenantId, analysis },
        "Completed line of service clustering"
      );
      return analysis;
    } catch (error) {
      logger.error(
        { error, tenantId },
        "Failed to cluster line of service data"
      );
      throw error;
    }
  }
  ...
  async standardizeAllData(tenantId: string = "default"): Promise<{
    ...
    try {
      ...
      const totalItems =
        lineOfService.totalItems + suppliers.totalItems + roles.totalItems;
      const clusteredItems =
        lineOfService.clusteredItems +
        suppliers.clusteredItems +
        roles.clusteredItems;
      const clustersCreated =
        lineOfService.clusters.length +
        suppliers.clusters.length +
        roles.clusters.length;

      const summary = {
        totalItemsProcessed: totalItems,
        standardizationRate:
          totalItems > 0 ? (clusteredItems / totalItems) * 100 : 0,
        clustersCreated,
        recommendations: [
      ...
      logger.info(
        { tenantId, summary },
        "Completed comprehensive data standardization"
      );
  ...
  private async findDatabaseMapping(
    category: string,
    value: string
  ): Promise<StandardizationResult | null> {
    try {
  ...
  private async detectSupplierVariations(
    normalized: string
  ): Promise<StandardizationResult | null> {
  ...
  private extractRoleComponents(value: string): {
    ...
    const role = words.find((word) =>
      roleKeywords.some((keyword) => word.includes(keyword))
    );
    const specialization = words.find(
      (word) =>
  ...
  private extractSeniorityLevel(
    ...
    if (context?.experience) {
      if (context.experience < 3)
        return { level: "Junior", confidence: 0.7, alternatives: ["Mid"] };
      if (context.experience < 7)
        return { level: "Mid", confidence: 0.7, alternatives: ["Senior"] };
      if (context.experience < 12)
        return { level: "Senior", confidence: 0.7, alternatives: ["Lead"] };
    ...
    return {
      level: "Mid",
      confidence: 0.5,
      alternatives: ["Junior", "Senior"],
    };
  }

  private async performClustering(
    data: Array<{ value: string; frequency: number }>,
    category: string
    ...
    for (const item of data) {
      ...
      for (const otherItem of data) {
        if (otherItem.value === item.value || processed.has(otherItem.value))
          continue;
  ...
  private async performSupplierClustering(
    data: Array<{ supplier_name: string; frequency: number }>
  ): Promise<ClusterDefinition[]> {
    ...
    for (const item of data) {
      ...
      for (const otherItem of data) {
        if (
          otherItem.supplier_name === item.supplier_name ||
          processed.has(otherItem.supplier_name)
        )
          continue;

        const otherNormalized = this.normalizeSupplierName(
          otherItem.supplier_name
        );
        const similarity = this.calculateSimilarity(
          normalized,
          otherNormalized
  ...
  private async performRoleClustering(
    data: Array<{ role: string; level?: string; frequency: number }>
  ): Promise<ClusterDefinition[]> {
    ...
    for (const item of data) {
      ...
      for (const otherItem of data) {
        ...
        const similarity = this.calculateRoleSimilarity(
          components,
          otherComponents
        );
  ...
  private calculateRoleSimilarity(components1: any, components2: any): number {
    ...
    if (components1.role && components2.role) {
      score +=
        this.calculateSimilarity(components1.role, components2.role) * 0.6;
      factors += 0.6;
    }
    ...
    if (components1.specialization && components2.specialization) {
      score +=
        this.calculateSimilarity(
          components1.specialization,
          components2.specialization
        ) * 0.1;
      factors += 0.1;
    }
  ...
  private findUnclusteredItems(
    ...
    return rawData
      .filter(
        (item) =>
          !clusteredItems.has(
            item.supplier_name || item.line_of_service || item.role
          )
      )
  ...
  private generateClusteringRecommendations(
    ...
    if (clusters.length > 20) {
      recommendations.push(
        "Consider consolidating similar categories to reduce complexity"
      );
    }
    ...
    if (lowConfidenceClusters.length > 0) {
      recommendations.push(
        `Review ${lowConfidenceClusters.length} low-confidence clusters for accuracy`
      );
    }

    recommendations.push(
      "Implement automated standardization rules for future data"
    );
  ...
  private async storeStandardizationResults(
    tenantId: string,
    results: any
  ): Promise<void> {
  ...
  async healthCheck(): Promise<boolean> {
    try {
      ...
      const testResult = await this.standardizeLineOfService(
      ...
      logger.error(
        { error },
        "Data standardization service health check failed"
      );
      return false;
    }
  }
...
export const dataStandardizationService =
  DataStandardizationService.getInstance();
