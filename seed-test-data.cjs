...
const prisma = new PrismaClient({
  datasources: {
    db: {
      url: process.env.DATABASE_URL || 'postgresql://postgres:postgres@localhost:5432/contract_intelligence?schema=public'
    }
  }
});
...
async function main() {
  ...
  const contracts = [
    {
      ...
      description: 'Enterprise software licensing agreement with ACME Corporation',
      category: 'Software',
    ...
    },
    {
      ...
      description: 'Mutual NDA for partnership discussions and confidential information exchange',
      category: 'Legal',
      contractType: 'NDA',
      status: 'COMPLETED',
  ...
  for (const contractData of contracts) {
    ...
    // Create sample artifacts for each contract
    const artifactTypes = ['OVERVIEW', 'FINANCIAL', 'PARTIES', 'DATES', 'CLAUSES'];
    for (let i = 0; i < 3; i++) {
      await prisma.artifact.create({
        data: {
          ...
          name: `${artifactTypes[i % artifactTypes.length].toLowerCase()}-analysis`,
          data: {
            confidence: 0.95,
            extractedAt: new Date().toISOString(),
            summary: `Sample ${artifactTypes[i % artifactTypes.length]} data for ${contract.contractTitle}`
          },
    ...
    console.log(`   📎 Created ${3} artifacts`);
  }

  console.log('');
  console.log('🎉 Test data seeded successfully!');
  console.log(`📊 Total contracts: ${contracts.length}`);
  console.log(`📎 Total artifacts: ${contracts.length * 3}`);
}
