...
export const HelpIcon = memo(({
  ...
  const variantClasses = {
    ...
    prominent: 'text-indigo-600 bg-indigo-100 dark:bg-indigo-900/30 p-1 rounded-full',
  };
                ...
                <ol className="mt-3 space-y-2">
                  {tip.steps.map((step, i) => (
                    <li key={i} className="flex items-start gap-2 text-sm">
                      <span className="flex-shrink-0 w-5 h-5 rounded-full bg-indigo-100 dark:bg-indigo-900/30 text-indigo-600 dark:text-indigo-400 flex items-center justify-center text-xs font-medium">
                        {i + 1}
                      </span>
...
export const FeatureSpotlight = memo(({
  ...
  return (
    <div className={cn("relative", className)}>
      {/* Highlight ring */}
      <div className="absolute -inset-2 rounded-xl border-2 border-indigo-500 animate-pulse pointer-events-none" />
        ...
        <div className="p-4">
          <div className="flex items-start justify-between mb-2">
            <div className="flex items-center gap-2">
              {isNew && (
                <span className="px-1.5 py-0.5 text-[10px] font-bold uppercase tracking-wider text-white bg-gradient-to-r from-indigo-500 to-purple-500 rounded">
                  New
                </span>
              )}
