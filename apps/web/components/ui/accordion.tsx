...
>(({ className, children, ...props }, ref) => (
  <AccordionPrimitive.Header className="flex">
    <AccordionPrimitive.Trigger
      ref={ref}
      className={cn(
        "flex flex-1 items-center justify-between py-4 font-semibold text-slate-900 dark:text-white transition-all hover:text-violet-600 dark:hover:text-violet-400 [&[data-state=open]>svg]:rotate-180 [&[data-state=open]]:text-violet-600 dark:[&[data-state=open]]:text-violet-400",
        className
      )}
      {...props}
    >
