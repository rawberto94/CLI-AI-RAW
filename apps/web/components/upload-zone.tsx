...
export function UploadZone({ 
  ...
  const uploadFiles = async () => {
    ...
    for (const uploadFile of files.filter(f => f.status === 'pending')) {
      try {
        ...
        if (!['application/pdf', 'application/msword', 'application/vnd.openxmlformats-officedocument.wordprocessingml.document'].includes(uploadFile.file.type)) {
  ...
  const pollForCompletion = async (docId: string, fileId: string) => {
    ...
    while (attempts < maxAttempts) {
      try {
        ...
        if (response.ok) {
          ...
          setFiles(prev => prev.map(f => 
            f.id === fileId 
              ? { ...f, progress: Math.max(50, Math.min(95, 50 + (percent * 0.45))) }
              : f
          ));
    ...
    if (attempts >= maxAttempts) {
      setFiles(prev => prev.map(f => 
        f.id === fileId 
          ? { ...f, status: 'error', error: 'Processing timeout - please check document status' }
          : f
      ));
    }
  };
