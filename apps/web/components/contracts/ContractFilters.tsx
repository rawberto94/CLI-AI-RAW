      ...
      {totalActiveFilters > 0 && (
        <div className="flex flex-wrap items-center gap-2 ml-auto">
          {selectedStatuses.map(status => (
            <FilterChip
              key={`status-${status}`}
              label={CONTRACT_STATUS_OPTIONS.find(o => o.value === status)?.label || status}
              color={CONTRACT_STATUS_OPTIONS.find(o => o.value === status)?.color}
          ...
          {selectedTypes.map(type => (
            <FilterChip
              key={`type-${type}`}
              label={CONTRACT_TYPE_OPTIONS.find(o => o.value === type)?.label || type}
          ...
          {selectedRiskLevels.map(level => (
            <FilterChip
              key={`risk-${level}`}
              label={RISK_LEVEL_OPTIONS.find(o => o.value === level)?.label || level}
