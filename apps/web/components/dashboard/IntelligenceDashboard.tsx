...
export default function IntelligenceDashboard() {
  ...
  const fetchDashboardData = async (showRefreshing = false) => {
    try {
      ...
      const response = await fetch('/api/intelligence/comprehensive?tenantId=demo')
      ...
      if (result.success) {
        ...
        const dashboardData: DashboardData = {
          ...
          trends: {
            ...
            processingEfficiency: result.data.portfolio.performance.processingEfficiency,
  ...
  const getActivityIcon = (type: string) => {
    switch (type) {
      case 'pattern_detected': return <Target className="w-4 h-4 text-blue-500" />
      case 'insight_generated': return <Lightbulb className="w-4 h-4 text-yellow-500" />
      case 'contract_processed': return <FileText className="w-4 h-4 text-green-500" />
      ...
      <div className="space-y-6">
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-gray-600 mt-1">AI-powered contract insights and analytics</p>
          </div>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        <Card className="shadow-lg border-0 bg-gradient-to-br from-blue-50 to-blue-100">
          <CardContent className="p-6">
            <div className="flex items-center justify-between">
              <div>
                <p className="text-sm font-medium text-blue-700">Total Contracts</p>
                <p className="text-3xl font-bold text-blue-900">
                  {formatNumber(data.overview.totalContracts)}
                </p>
                <div className="flex items-center gap-1 mt-1">
                  {getTrendIcon(data.trends.contractVelocity)}
                  <span className={`text-xs ${getTrendColor(data.trends.contractVelocity)}`}>
                    {data.trends.contractVelocity > 0 ? '+' : ''}{data.trends.contractVelocity} this month
                  </span>
                </div>
              </div>
            ...
            <div className="flex items-center justify-between">
              <div>
                ...
                <div className="flex items-center gap-1 mt-1">
                  ...
                  <span className={`text-xs ${getTrendColor(data.trends.valueVelocity)}`}>
        ...
        <Card className="shadow-lg border-0 bg-gradient-to-br from-orange-50 to-orange-100">
          <CardContent className="p-6">
            <div className="flex items-center justify-between">
              <div>
                <p className="text-sm font-medium text-orange-700">Processing Efficiency</p>
            ...
            {data.recommendations.length > 0 ? (
              <div className="space-y-4">
                {data.recommendations.slice(0, 3).map((rec) => (
                  <div key={rec.id} className="flex items-start gap-4 p-4 bg-white rounded-lg border border-gray-200 hover:shadow-md transition-shadow">
                    <div className={`p-2 rounded-lg ${
                      rec.priority === 'high' ? 'bg-red-100' :
                      rec.priority === 'medium' ? 'bg-yellow-100' : 'bg-blue-100'
                    }`}>
                      {rec.type === 'urgent' ? <AlertTriangle className="w-5 h-5 text-red-600" /> :
                       rec.type === 'cost_optimization' ? <DollarSign className="w-5 h-5 text-green-600" /> :
              ...
              <div className="text-center py-8">
                ...
                <p className="text-gray-400 text-sm">Upload more contracts to generate insights</p>
              </div>
            )}
          </CardContent>
        </Card>
        ...
        <CardContent className="p-6">
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            <div className="text-center">
              ...
              <div className="text-xs text-gray-500">AI-identified patterns in contracts</div>
            </div>
            ...
            <div className="text-center">
              ...
              <div className="text-sm text-gray-600 mb-1">Processing Efficiency</div>
              <div className="text-xs text-gray-500">Successful analysis rate</div>
            </div>
          </div>
          
          <div className="mt-6 pt-6 border-t text-center">
            <p className="text-sm text-gray-500">
              Last analysis: {new Date(data.intelligence.lastAnalysisTime).toLocaleString()}
            </p>
          </div>
        </CardContent>
      </Card>
