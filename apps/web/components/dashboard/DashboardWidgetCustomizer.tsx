...
const CATEGORY_LABELS: Record<string, { label: string; color: string }> = {
  ...
  workflows: { label: 'Workflows', color: 'bg-violet-100 text-violet-700' },
  ...
  return (
    ...
    >
      ...
      <div className={`w-10 h-10 rounded-lg flex items-center justify-center ${
        widget.isVisible ? 'bg-violet-100 text-violet-600' : 'bg-slate-200 text-slate-400'
      }`}>
              ...
              {(['small', 'medium', 'large', 'full'] as const).map((size) => (
                <button
                  ...
                  className={`w-full px-3 py-1.5 text-left text-xs hover:bg-slate-50 flex items-center justify-between ${
                    widget.size === size ? 'text-violet-600 bg-violet-50' : 'text-slate-600'
                  }`}
                >
                  {SIZE_LABELS[size]}
        ...
        <div className="px-6 py-4 border-t border-slate-200 flex items-center justify-between shrink-0 bg-slate-50">
          ...
          <div className="flex items-center gap-3">
            ...
            <button
              onClick={handleSave}
              disabled={!hasChanges}
              className="flex items-center gap-2 px-5 py-2.5 bg-gradient-to-r from-violet-500 to-purple-600 text-white font-medium rounded-lg hover:shadow-lg hover:shadow-violet-200 disabled:opacity-50 disabled:cursor-not-allowed transition-all"
            >
...
export const DashboardCustomizeButton: React.FC<{
  ...
  <button
    onClick={onClick}
    className={`flex items-center gap-2 px-3 py-2 text-sm font-medium text-slate-600 hover:text-violet-600 hover:bg-violet-50 rounded-lg transition-colors ${className || ''}`}
  >
