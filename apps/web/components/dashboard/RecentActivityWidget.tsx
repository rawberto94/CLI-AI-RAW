...
const ACTIVITY_CONFIG: Record<ActivityType, {
  ...
  contract_created: { icon: FileText, color: 'text-emerald-500', bgColor: 'bg-emerald-50' },
  contract_updated: { icon: Edit, color: 'text-blue-500', bgColor: 'bg-blue-50' },
  ...
  document_uploaded: { icon: Upload, color: 'text-indigo-500', bgColor: 'bg-indigo-50' },
  document_downloaded: { icon: Download, color: 'text-cyan-500', bgColor: 'bg-cyan-50' },
  approval_requested: { icon: Clock, color: 'text-amber-500', bgColor: 'bg-amber-50' },
  approval_completed: { icon: CheckCircle2, color: 'text-emerald-500', bgColor: 'bg-emerald-50' },
  approval_rejected: { icon: AlertCircle, color: 'text-red-500', bgColor: 'bg-red-50' },
  comment_added: { icon: MessageSquare, color: 'text-blue-500', bgColor: 'bg-blue-50' },
  ...
  status_changed: { icon: Activity, color: 'text-teal-500', bgColor: 'bg-teal-50' },
...
function ActivityItemRow({ activity, variant, showContract = true }: ActivityItemRowProps) {
  ...
  return (
    ...
    >
      {/* New indicator */}
      {activity.isNew && (
        <div className="absolute left-0 top-1/2 -translate-y-1/2 w-1.5 h-1.5 rounded-full bg-indigo-500" />
      )}
          ...
          {showContract && activity.contract && (
            <>
              ...
              <Link
                ...
                className={cn(
                  "hover:text-indigo-600 hover:underline truncate max-w-[120px]",
                  isCompact ? "text-[10px]" : "text-xs"
                )}
              >
      ...
      {activity.contract && (
        ...
        >
          <ArrowUpRight className="h-4 w-4 text-slate-400 hover:text-indigo-500" />
        </Link>
      )}
    </motion.div>
  )
...
export function generateDemoActivities(count: number = 10): ActivityItem[] {
  const activityTypes: ActivityType[] = [
    'contract_created', 'contract_updated', 'version_created',
    'approval_requested', 'approval_completed', 'comment_added',
    'document_uploaded', 'ai_analysis', 'status_changed'
  ]
  ...
  const users = [
    ...
    { id: '4', name: 'System', email: 'system@contigo.ai' },
  ]
  
  const contracts = [
    { id: 'c1', name: 'Microsoft EA 2024' },
    { id: 'c2', name: 'AWS Services Agreement' },
    { id: 'c3', name: 'Salesforce Enterprise' },
    { id: 'c4', name: 'Google Cloud Contract' },
  ]
  
  const titles: Record<ActivityType, string[]> = {
    contract_created: ['Created new contract', 'Added contract to repository'],
    contract_updated: ['Updated contract details', 'Modified contract metadata'],
    contract_viewed: ['Viewed contract'],
    contract_deleted: ['Deleted contract'],
    version_created: ['Created version snapshot', 'Uploaded new version'],
    version_reverted: ['Reverted to previous version'],
    document_uploaded: ['Uploaded supporting document', 'Added attachment'],
    document_downloaded: ['Downloaded contract document'],
    approval_requested: ['Requested approval', 'Submitted for review'],
    approval_completed: ['Approved contract', 'Completed approval'],
    approval_rejected: ['Rejected contract', 'Declined approval'],
    comment_added: ['Added comment', 'Left feedback'],
    shared: ['Shared contract', 'Invited collaborator'],
    reminder_set: ['Set renewal reminder', 'Added deadline alert'],
    status_changed: ['Changed status', 'Updated contract state'],
    value_updated: ['Updated contract value', 'Modified financial terms'],
    ai_analysis: ['AI completed analysis', 'Generated insights'],
    risk_detected: ['Risk alert triggered', 'Compliance issue detected'],
  }
  ...
  return Array.from({ length: count }, (_, i) => {
    ...
    return {
      ...
      description: `Activity on ${contract.name}`,
