...
exports.ExtractionAnalyticsService = void 0;
exports.getExtractionAnalytics = getExtractionAnalytics;
// ============================================================================
// ANALYTICS SERVICE
// ============================================================================
class ExtractionAnalyticsService {
     ...
     */
    async recordExtractionStart(contractId, tenantId) {
     ...
     */
    async recordExtractionComplete(contractId, tenantId, fields, processingTimeMs, modelUsed) {
     ...
     */
    async recordExtractionFailed(contractId, tenantId, errorMessage, processingTimeMs) {
        await this.recordEvent({
            contractId,
            tenantId,
     ...
     */
    async recordFieldAutoApplied(contractId, tenantId, fieldKey, fieldType, value, confidence) {
        await this.recordEvent({
            contractId,
            tenantId,
     ...
     */
    async recordFieldCorrected(contractId, tenantId, fieldKey, fieldType, originalValue, correctedValue, originalConfidence) {
        await this.recordEvent({
            contractId,
            tenantId,
     ...
     */
    async recordFieldRejected(contractId, tenantId, fieldKey, fieldType, rejectedValue, originalConfidence) {
        await this.recordEvent({
            contractId,
            tenantId,
     ...
     */
    async getGlobalAnalytics(startDate, endDate) {
        ...
        for (const event of periodEvents) {
            if (event.modelUsed && (event.eventType === "extraction_completed" || event.eventType === "extraction_failed")) {
                ...
                if (event.processingTimeMs)
                    stats.totalTime += event.processingTimeMs;
    ...
    async getRecommendations(tenantId) {
        ...
        if (analytics.averageConfidence < 0.6) {
            recommendations.push("Consider providing more context in contract documents or simplifying field definitions");
        }
        ...
        for (const metric of analytics.fieldMetrics) {
            if (metric.correctionRate > 0.3 && metric.totalExtractions > 10) {
                recommendations.push(`Field type "${metric.fieldType}" has a ${Math.round(metric.correctionRate * 100)}% correction rate. ` +
        ...
        if (lowAccuracyFields.length > 0) {
            ...
            recommendations.push(`Focus on improving extraction for: ${fieldNames}`);
        }
        // Slow processing
        if (analytics.averageProcessingTime > 30000) {
            recommendations.push("Average processing time is high. Consider using faster models for initial extraction " +
     ...
     */
    async getRealTimeInsights(tenantId) {
        ...
        const avgConfidence = this.calculateAverage(recentEvents.filter(e => e.confidence !== undefined).map(e => e.confidence));
        const avgProcessingTime = this.calculateAverage(recentEvents.filter(e => e.processingTimeMs !== undefined).map(e => e.processingTimeMs));
        // Calculate trends
        ...
        const last24hConfidence = this.calculateAverage(last24h.filter(e => e.confidence !== undefined).map(e => e.confidence));
        const prev24hConfidence = this.calculateAverage(previous24h.filter(e => e.confidence !== undefined).map(e => e.confidence));
        const last24hTime = this.calculateAverage(last24h.filter(e => e.processingTimeMs !== undefined).map(e => e.processingTimeMs));
        const prev24hTime = this.calculateAverage(previous24h.filter(e => e.processingTimeMs !== undefined).map(e => e.processingTimeMs));
        const determineTrend = (current, previous, threshold = 0.05) => {
...
exports.ExtractionAnalyticsService = ExtractionAnalyticsService;
// ============================================================================
// SINGLETON
// ============================================================================
