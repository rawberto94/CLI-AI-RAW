...
const DEFAULT_FIELDS: Omit<MetadataFieldDefinition, 'id' | 'createdAt' | 'updatedAt'>[] = [
  ...
  },
  {
    ...
    validations: [],
    options: [
      { value: 'signed', label: 'Fully Signed', color: 'green' },
      { value: 'partially_signed', label: 'Partially Signed', color: 'yellow' },
      { value: 'unsigned', label: 'Unsigned', color: 'red' },
      { value: 'unknown', label: 'Unknown', color: 'gray' },
    ],
    defaultValue: 'unknown',
    aiExtractionEnabled: true,
    aiExtractionHint: 'Determine if the contract is signed by analyzing: (1) Signature blocks - look for actual signatures, names with dates, or signature lines that are filled vs empty; (2) Language like "duly executed", "signed and delivered", "IN WITNESS WHEREOF"; (3) Multiple party signatures - check if all parties have signed; (4) Electronic signature indicators like DocuSign, Adobe Sign timestamps; (5) Draft watermarks or "EXECUTION COPY" markings. Return "signed" if all parties have signed, "partially_signed" if some but not all, "unsigned" if no signatures found, "unknown" if cannot determine.',
  ...
  },
  {
    ...
    defaultValue: false,
    aiExtractionEnabled: true,
    aiExtractionHint: 'Set to TRUE if: (1) Contract is unsigned or partially signed, (2) There are empty signature lines, (3) Contract appears to be a draft, (4) Missing execution dates, (5) Any indication that signatures are still needed. This flags contracts that need signature attention.',
    showInList: true,
    showInCard: true,
    filterable: true,
    width: 'half',
