...
import { createErrorResponse, handleApiError } from '@/lib/api-middleware';
  ...
  const document = new Document({
    sections: [{
      children: [
        ...
        new Table({
          rows: [
            makeRow('Title', contract.contractTitle || 'N/A'),
            makeRow('Status', contract.status),
            makeRow('Type', contract.contractType || 'N/A'),
            makeRow('Client', contract.clientName || 'N/A'),
            makeRow('Supplier', contract.supplierName || 'N/A'),
            makeRow('Total Value', formatValue(contract.totalValue ? Number(contract.totalValue) : null, contract.currency)),
            makeRow('Start Date', formatDate(contract.startDate)),
            makeRow('End Date', formatDate(contract.endDate)),
            makeRow('Expiration', formatDate(contract.expirationDate)),
            makeRow('Jurisdiction', contract.jurisdiction || 'N/A'),
            makeRow('Uploaded', formatDate(contract.uploadedAt)),
          ],
...
function formatArtifactData(data: string | Record<string, unknown> | unknown[]): string {
  ...
  if (typeof data === 'object' && data !== null) {
    return `<table>
      ${Object.entries(data).map(([key, value]) => `<tr><td class="label">${key}</td><td>${typeof value === 'object' ? JSON.stringify(value) : value}</td></tr>`).join('')}
    </table>`;
  }
  return String(data);
}
...
function generatePDFContent(contract: ContractExportData): string {
  ...
  const summary = artifacts.find((artifact: ExportArtifact) => artifact.type === 'summary');
  const keyTerms = artifacts.find((artifact: ExportArtifact) => artifact.type === 'key_terms');
  const obligations = artifacts.find((artifact: ExportArtifact) => artifact.type === 'obligations');
  const risks = artifacts.find((artifact: ExportArtifact) => artifact.type === 'risks');
...
export async function getContractExport(
  request: NextRequest,
    ...
    switch (format) {
      case 'json':
        return new NextResponse(JSON.stringify(contract, null, 2), {
          headers: {
            ...
            'Content-Disposition': `attachment; filename="contract-${contractId}.json"`,
          },
        });
      ...
      case 'pdf': {
        const pdfBytes = generateContractReportPDF({
          ...
          artifacts: (contract.artifacts || []).map((artifact: ExportArtifact) => ({
            type: artifact.type,
            data: typeof artifact.data === 'string'
              ? safeJsonParse(artifact.data)
      ...
      case 'html':
        return new NextResponse(generatePDFContent(contract), {
          headers: {
      ...
      case 'csv': {
        ...
        return new NextResponse(new Uint8Array(xlsxBuffer), {
          headers: {
            ...
            'Content-Disposition': `attachment; filename="contract-${contractId}.${isCsv ? 'csv' : 'xls'}"`,
          },
        });
      }
      ...
      default:
        return createErrorResponse(
          context,
          'BAD_REQUEST',
          'Invalid format. Supported formats: json, pdf, docx, html, excel, csv',
          400,
        );
    }
  } catch (error: unknown) {
