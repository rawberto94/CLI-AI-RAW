import { TalkingPoint, MarketIntelligence, TrendAnalysis, TargetRates } from './rate-history-types'
...
export class TalkingPointsGenerator {
  ...
  private static generateMarketPoints(config: TalkingPointsConfig): TalkingPoint[] {
    const points: TalkingPoint[] = []
    const { marketIntelligence, currentRate, targetRate } = config
    
    // Market position argument
    if (marketIntelligence.competitivePosition.marketRank > 60) {
      const percentile = marketIntelligence.competitivePosition.marketRank
      points.push({
        id: `market-position-${Date.now()}`,
        category: 'market',
        title: 'Current Rate Above Market Average',
        text: `Our current rate of CHF ${currentRate.toLocaleString()} places us in the ${percentile}th percentile of the market. This means we're paying more than ${percentile}% of similar engagements. Market data shows the median rate is CHF ${marketIntelligence.marketStats.median.toLocaleString()}, representing a ${Math.round(((currentRate - marketIntelligence.marketStats.median) / currentRate) * 100)}% premium.`,
        supportingData: `Market analysis of ${marketIntelligence.marketStats.count} comparable rates`,
    ...
    const competitorsBelow = marketIntelligence.competitivePosition.competitorsBelow
    if (competitorsBelow > 0) {
      points.push({
        id: `competitive-benchmark-${Date.now()}`,
        category: 'competitive',
        title: 'Multiple Suppliers Offer Lower Rates',
        text: `Market analysis reveals ${competitorsBelow} suppliers offering comparable ${config.role} resources at lower rates. The target rate of CHF ${targetRate.toLocaleString()} aligns with the 25th percentile, which is a reasonable market position while maintaining quality standards.`,
    ...
    const avgDiff = currentRate - marketIntelligence.marketStats.average
    if (avgDiff > 0) {
      points.push({
        id: `market-average-${Date.now()}`,
        category: 'market',
        title: 'Rate Exceeds Market Average',
        text: `The current rate exceeds the market average by CHF ${Math.round(avgDiff).toLocaleString()} per day (${Math.round((avgDiff / currentRate) * 100)}%). Aligning with market averages would result in more competitive pricing while maintaining service quality.`,
        supportingData: `Market average: CHF ${marketIntelligence.marketStats.average.toLocaleString()}`,
  ...
  private static generateVolumePoints(config: TalkingPointsConfig): TalkingPoint[] {
    const points: TalkingPoint[] = []
    ...
    if (volume >= 200) {
      ...
      points.push({
        ...
        text: `We're committing to ${volume} person-days annually, representing CHF ${annualValue.toLocaleString()} in annual business. This substantial volume warrants preferential pricing. A rate reduction to CHF ${config.targetRate.toLocaleString()} would still maintain CHF ${(config.targetRate * volume).toLocaleString()} in annual revenue for the supplier.`,
    ...
    if (volume >= 150) {
      points.push({
        ...
        text: `Industry standard volume discounts for engagements of ${volume}+ person-days typically range from 10-15%. Our target rate represents a ${Math.round(((config.currentRate - config.targetRate) / config.currentRate) * 100)}% reduction, which is within standard volume discount ranges.`,
  ...
  private static generateCompetitivePoints(config: TalkingPointsConfig): TalkingPoint[] {
    const points: TalkingPoint[] = []
    ...
    points.push({
      ...
      text: `We have identified alternative suppliers offering ${config.role} (${config.level}) resources in ${config.location} at rates between CHF ${targetRates.market.p10.toLocaleString()} and CHF ${targetRates.market.p25.toLocaleString()}. While we value our current partnership, we need to ensure competitive pricing to justify continued engagement.`,
    ...
    const savingsPercent = Math.round(((currentRate - targetRate) / currentRate) * 100)
    points.push({
  ...
  private static generateTrendPoints(config: TalkingPointsConfig): TalkingPoint[] {
    const points: TalkingPoint[] = []
    ...
    if (trendAnalysis.direction === 'decreasing') {
      points.push({
        ...
        text: `Historical analysis shows ${config.role} rates in ${config.location} have been declining at CHF ${Math.abs(trendAnalysis.slope).toFixed(0)} per month over the past year. This downward trend is expected to continue, with 6-month forecasts predicting rates around CHF ${trendAnalysis.forecast.sixMonth.toLocaleString()}. Adjusting rates now aligns with market trajectory.`,
        supportingData: `Trend: ${trendAnalysis.slope.toFixed(1)} CHF/month decline`,
        persuasivenessScore: 8,
    ...
    if (trendAnalysis.direction === 'stable' && trendAnalysis.volatility < 10) {
      points.push({
        ...
        text: `Market analysis shows stable rate conditions with low volatility (${trendAnalysis.volatility.toFixed(1)}%). This stability provides confidence that the target rate of CHF ${config.targetRate.toLocaleString()} is sustainable and reflects true market value rather than temporary fluctuations.`,
  ...
  private static generateRelationshipPoints(config: TalkingPointsConfig): TalkingPoint[] {
    const points: TalkingPoint[] = []
    ...
    points.push({
      ...
      text: `A rate adjustment to CHF ${config.targetRate.toLocaleString()} maintains substantial value for both parties. The supplier retains CHF ${(config.targetRate * annualVolume).toLocaleString()} in annual revenue with guaranteed volume, while we achieve market-competitive pricing. This creates a sustainable, win-win partnership.`,
      supportingData: `Retained annual value: CHF ${(config.targetRate * annualVolume).toLocaleString()}`,
      persuasivenessScore: 8,
  ...
  static generateFallbackPoints(config: Partial<TalkingPointsConfig>): TalkingPoint[] {
    return [
      {
...
export const TalkingPointsUtils = {
  ...
  getCategoryStyle(category: TalkingPoint['category']): {
    ...
    const styles = {
      market: { color: 'text-blue-600', bgColor: 'bg-blue-50', label: 'Market Data' },
      volume: { color: 'text-green-600', bgColor: 'bg-green-50', label: 'Volume' },
      competitive: { color: 'text-purple-600', bgColor: 'bg-purple-50', label: 'Competitive' },
      performance: { color: 'text-orange-600', bgColor: 'bg-orange-50', label: 'Performance' },
      relationship: { color: 'text-pink-600', bgColor: 'bg-pink-50', label: 'Relationship' }
    }
  ...
  getPersuasivenessBadge(score: number): {
    ...
    if (score >= 8) return { label: 'Strong', className: 'bg-green-100 text-green-800' }
    if (score >= 6) return { label: 'Moderate', className: 'bg-yellow-100 text-yellow-800' }
  ...
  exportToText(points: TalkingPoint[]): string {
    ...
    points.forEach((point, index) => {
      ...
      text += `   Strength: ${this.getPersuasivenessBadge(point.persuasivenessScore).label}\n\n`
      text += `   ${point.text}\n\n`
