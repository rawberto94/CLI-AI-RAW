      ...
      if (dashboardData) {
        ...
        const mockData: AnalyticsData = {
          ...
          rateCard: {
            ...
            avgVariance: 12.3,
            topOpportunities: [
              { supplier: 'Accenture', category: 'IT Services', potentialSavings: 340000, confidence: 0.92 },
              { supplier: 'Deloitte', category: 'Consulting', potentialSavings: 280000, confidence: 0.87 },
              { supplier: 'PwC', category: 'Advisory', potentialSavings: 195000, confidence: 0.84 }
            ]
          },
          renewals: {
            totalRenewals: 67,
            ...
            avgDaysToExpiry: 145,
            upcomingRenewals: [
              { contractId: 'CNT-001', supplier: 'Microsoft', expiryDate: '2024-04-15', riskLevel: 'high', value: 2400000 },
              { contractId: 'CNT-002', supplier: 'Oracle', expiryDate: '2024-05-20', riskLevel: 'medium', value: 1800000 },
              { contractId: 'CNT-003', supplier: 'SAP', expiryDate: '2024-06-10', riskLevel: 'low', value: 950000 }
            ]
          },
          compliance: {
            averageScore: 82.4,
            ...
            topIssues: [
              { issue: 'Missing liability clauses', frequency: 23, impact: 'high' },
              { issue: 'Weak termination terms', frequency: 18, impact: 'medium' },
              { issue: 'GDPR compliance gaps', frequency: 12, impact: 'critical' }
            ]
          },
          suppliers: {
            totalSuppliers: 156,
            topPerformers: [
              { name: 'Accenture', score: 94, riskLevel: 'low', contractValue: 8500000 },
              { name: 'Deloitte', score: 91, riskLevel: 'low', contractValue: 6200000 },
              { name: 'IBM', score: 88, riskLevel: 'medium', contractValue: 4800000 }
            ],
            riskAlerts: 8
          },
          nlq: {
    ...
    return (
      <div className="p-6">
        <div className="text-center">
          ...
          <p className="text-gray-600 mb-4">Unable to load analytical intelligence data</p>
  ...
  return (
    <div className="p-6 space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-3xl font-bold text-gray-900">Analytical Intelligence</h1>
                  ...
                  {queryResult.evidence && queryResult.evidence.length > 0 && (
                    <div className="text-sm text-gray-600">
                      ...
                      {queryResult.evidence.map((evidence: any, index: number) => (
                        <div key={index} className="ml-2">• {evidence.excerpt}</div>
                      ))}
                    </div>
                  )}
      ...
      <Tabs defaultValue="overview" className="space-y-6">
        ...
        <TabsContent value="overview" className="space-y-6">
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            <EnhancedCard title="Contract Portfolio Health" subtitle="Overall portfolio metrics">
              <div className="text-center space-y-4">
                <ScoreGauge score={data.overview.complianceScore} size="lg" label="Portfolio Health" />
                <div className="grid grid-cols-2 gap-4 text-center">
                  <div>
                    <div className="text-2xl font-bold text-green-600">${(data.overview.avgSavings * data.overview.totalValue / 100 / 1000000).toFixed(1)}M</div>
                    <div className="text-sm text-gray-600">Potential Savings</div>
                  </div>
                  <div>
                    <div className="text-2xl font-bold text-blue-600">{data.overview.renewalsNext90Days}</div>
            ...
            <EnhancedCard title="Key Performance Indicators" subtitle="Critical metrics at a glance">
              <div className="space-y-4">
                <div className="flex items-center justify-between">
                  <span className="text-sm text-gray-600">Benchmarked Contracts</span>
                  <span className="font-medium">{Math.round(data.rateCard.benchmarkedContracts / data.overview.totalContracts * 100)}%</span>
                </div>
                <div className="flex items-center justify-between">
                  <span className="text-sm text-gray-600">Compliance Score</span>
                  <span className="font-medium">{data.compliance.averageScore}%</span>
                </div>
                <div className="flex items-center justify-between">
        ...
        <TabsContent value="benchmarking" className="space-y-6">
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            <EnhancedCard title="Savings Opportunities" subtitle="Top potential savings identified">
              <div className="space-y-4">
                {data.rateCard.topOpportunities.map((opportunity, index) => (
                  <div key={index} className="flex items-center justify-between p-3 bg-gray-50 rounded">
                    ...
                    <div className="text-right">
                      <div className="font-bold text-green-600">${(opportunity.potentialSavings / 1000).toFixed(0)}K</div>
                      <div className="text-xs text-gray-500">{Math.round(opportunity.confidence * 100)}% confidence</div>
                    </div>
                  </div>
                ))}
              </div>
            ...
            <EnhancedCard title="Benchmarking Summary" subtitle="Rate analysis overview">
              <div className="grid grid-cols-2 gap-4 text-center">
                <div className="p-3 bg-blue-50 rounded">
                  <div className="text-2xl font-bold text-blue-600">{data.rateCard.benchmarkedContracts}</div>
                  <div className="text-sm text-blue-700">Benchmarked</div>
                </div>
                <div className="p-3 bg-green-50 rounded">
                  <div className="text-2xl font-bold text-green-600">{data.rateCard.savingsOpportunities}</div>
                  <div className="text-sm text-green-700">Opportunities</div>
                </div>
                <div className="p-3 bg-orange-50 rounded">
                  <div className="text-2xl font-bold text-orange-600">{data.rateCard.avgVariance}%</div>
                  <div className="text-sm text-orange-700">Avg Variance</div>
                </div>
                <div className="p-3 bg-purple-50 rounded">
                  <div className="text-2xl font-bold text-purple-600">
                    ${(data.rateCard.topOpportunities.reduce((sum, opp) => sum + opp.potentialSavings, 0) / 1000000).toFixed(1)}M
                  </div>
        ...
        <TabsContent value="renewals" className="space-y-6">
          <div className="grid grid-cols-1 gap-6">
            <EnhancedCard title="Upcoming Renewals" subtitle="Contracts requiring attention">
              <div className="space-y-4">
                {data.renewals.upcomingRenewals.map((renewal, index) => (
                  <div key={index} className="flex items-center justify-between p-4 border rounded">
                    <div>
                      <div className="font-medium">{renewal.supplier}</div>
                      <div className="text-sm text-gray-600">Contract: {renewal.contractId}</div>
                      <div className="text-sm text-gray-600">Expires: {renewal.expiryDate}</div>
                    </div>
                    <div className="text-right">
                      <div className="font-bold">${(renewal.value / 1000000).toFixed(1)}M</div>
        ...
        <TabsContent value="compliance" className="space-y-6">
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            <EnhancedCard title="Compliance Overview" subtitle="Risk distribution and scores">
              <div className="text-center space-y-4">
                <ScoreGauge score={data.compliance.averageScore} size="lg" label="Compliance Score" />
                <div className="grid grid-cols-2 gap-4">
                  <div className="text-center">
                    <div className="text-2xl font-bold text-red-600">{data.compliance.criticalIssues}</div>
                    <div className="text-sm text-gray-600">Critical Issues</div>
                  </div>
                  <div className="text-center">
                    <div className="text-2xl font-bold text-green-600">
                      {data.compliance.riskDistribution.low + data.compliance.riskDistribution.medium}
                    </div>
            ...
            <EnhancedCard title="Top Compliance Issues" subtitle="Most frequent issues identified">
              <div className="space-y-3">
                {data.compliance.topIssues.map((issue, index) => (
                  <div key={index} className="flex items-center justify-between p-3 bg-gray-50 rounded">
                    <div>
                      <div className="font-medium">{issue.issue}</div>
                      <div className="text-sm text-gray-600">{issue.frequency} contracts affected</div>
                    </div>
        ...
        <TabsContent value="suppliers" className="space-y-6">
          <div className="grid grid-cols-1 gap-6">
            <EnhancedCard title="Top Performing Suppliers" subtitle="Highest rated suppliers by performance">
              <div className="space-y-4">
                  ...
                  <div key={index} className="flex items-center justify-between p-4 border rounded">
                    <div className="flex items-center gap-4">
                      ...
                      <div>
                        ...
                        <div className="text-sm text-gray-600">Score: {supplier.score}/100</div>
                      </div>
                    </div>
                    <div className="text-right">
                      <div className="font-bold">${(supplier.contractValue / 1000000).toFixed(1)}M</div>
        ...
        <TabsContent value="insights" className="space-y-6">
          <div className="text-center py-12">
            ...
            <p className="text-gray-600 mb-4">Advanced analytics and predictive insights coming soon...</p>
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4 max-w-2xl mx-auto">
              <div className="p-4 bg-blue-50 rounded border border-blue-200">
                <Zap className="w-6 h-6 text-blue-600 mx-auto mb-2" />
                <div className="text-sm font-medium text-blue-900">Predictive Analytics</div>
                <div className="text-xs text-blue-700">Contract risk prediction</div>
              </div>
              <div className="p-4 bg-green-50 rounded border border-green-200">
                <Target className="w-6 h-6 text-green-600 mx-auto mb-2" />
                <div className="text-sm font-medium text-green-900">Smart Recommendations</div>
                <div className="text-xs text-green-700">AI-driven optimization</div>
              </div>
