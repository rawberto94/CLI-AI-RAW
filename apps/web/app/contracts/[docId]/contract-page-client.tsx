...
// Client component that handles the main functionality
export function ContractPageClient({ params }: { params: { docId: string } }) {
  const { docId } = params;
  ...
  const fetchData = useCallback(async () => {
    try {
      const [presenceRes, contractRes, statusRes] = await Promise.all([
        ...
        fetch(`${API_BASE_URL}/api/contracts/${docId}/status`, { headers: tenantHeaders() }),
      ]);
  ...
  const reportUrl = useMemo(() => {
    ...
    return r?.storagePath ? `${API_BASE_URL}/api/contracts/${docId}/report.pdf` : undefined;
  }, [artifacts, docId]);
