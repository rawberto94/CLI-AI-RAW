            ...
            <CardContent className="pt-8 pb-8">
              <div className="text-center">
                ...
                <h2 className="text-lg sm:text-xl font-bold bg-gradient-to-r from-slate-900 to-slate-600 bg-clip-text text-transparent mb-2">
                  {isNotFound ? t('detail.notFoundTitle') : t('detail.errorTitle')}
                </h2>
    ...
    <div className="min-h-screen bg-gradient-to-br from-slate-50 via-purple-50/30 to-purple-50/20 dark:from-slate-950 dark:via-slate-900 dark:to-slate-950 print-container">
      ...
      <div className="max-w-[1600px] mx-auto px-4 sm:px-6 lg:px-8 pt-3 sm:pt-4 no-print">
        <div className="flex items-center justify-between">
          <Breadcrumbs
            items={[
              { label: t('title'), href: '/contracts', icon: FileText },
              { label: displayTitle },
            ]}
            showHomeIcon
          />
          ...
          <DialogContent className="sm:max-w-3xl">
            <DialogHeader>
              <div className="flex flex-wrap items-center gap-2 text-[11px] font-semibold uppercase tracking-[0.14em] text-violet-600">
                <span className="rounded-full bg-violet-100 px-2 py-0.5">[{citationRequest.index}] {t('detail.evidenceFocus')}</span>
                <span className="rounded-full bg-slate-100 px-2 py-0.5 text-slate-500">
                  {citationEvidence?.strategy === 'offset'
                    ? t('detail.spanExact')
                    : citationEvidence?.strategy === 'snippet'
                      ? t('detail.spanMatched')
                      : t('detail.spanFallback')}
                </span>
              </div>
              <DialogTitle className="mt-2 text-left">
                {citationRequest.heading || citationRequest.section || contractData?.contractTitle || contract?.filename || t('detail.contractEvidenceFallback')}
              </DialogTitle>
            </DialogHeader>
            ...
            <div className="space-y-4">
              ...
              <div className="rounded-2xl border border-slate-200 bg-slate-50 px-4 py-4">
                <p className="text-[11px] font-semibold uppercase tracking-[0.14em] text-slate-500">{t('detail.sourceContext')}</p>
      ...
      {showCategorySelector && (
        <Dialog open={showCategorySelector} onOpenChange={() => closeDialog('categorySelector')}>
          <DialogContent>
            <DialogHeader>
              <DialogTitle>{t('detail.selectCategoryTitle')}</DialogTitle>
            </DialogHeader>
            <CategorySelector
      ...
      {showExtendDialog && (
        <Dialog open={showExtendDialog} onOpenChange={() => closeDialog('extend')}>
          <DialogContent className="sm:max-w-lg">
            <DialogHeader>
              <DialogTitle>{t('detail.extendContractTitle')}</DialogTitle>
            </DialogHeader>
            <div className="space-y-4">
              {/* AI Recommendation */}
              {aiExtensionLoading && (
                <div className="flex items-center gap-2 text-sm text-violet-600">
                  ...
                  {t('detail.aiRecommendationLoading')}
                </div>
              )}
              {aiExtensionRec && (
                <Card className="border-violet-200 bg-violet-50/50">
                    ...
                    <div className="flex items-start gap-2">
                      ...
                      <div>
                        ...
                        {aiExtensionRec.suggestedExtensionMonths && (
                          <p className="text-xs text-violet-600 mt-1">{t('detail.suggestedMonths', { months: aiExtensionRec.suggestedExtensionMonths })}</p>
                        )}
                      </div>
                    </div>
                  </CardContent>
              ...
              <div className="flex justify-end gap-2">
                <Button variant="outline" onClick={() => closeDialog('extend')}>{tCommon('cancel')}</Button>
      ...
      <ConfirmDialog
        ...
        title={t('detail.deleteContractTitle')}
        description={t('detail.deleteContractDescription', { filename: contract?.filename || 'this contract' })}
        confirmLabel={t('detail.deleteContractTitle')}
        variant="destructive"
