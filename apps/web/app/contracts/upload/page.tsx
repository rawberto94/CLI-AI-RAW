...
import { UploadSuccessState, UploadErrorState, UploadMixedState } from "@/components/contracts/UploadStatusStates"
import { useUploadManager } from "@/lib/contracts/upload-manager"
import { API_BASE_URL } from "../../../lib/config"
import { tenantHeaders, getTenantId } from "../../../lib/tenant"
import { ArrowLeft, FileText, Upload as UploadIcon, Info, CheckCircle, AlertCircle, Sparkles, Zap, Shield } from "lucide-react"
...
export default function ContractsUploadPage() {
  ...
  const {
    ...
    onAllComplete: (results) => {
      ...
      if (successCount > 0 && failedCount === 0) {
        setTip(`Successfully uploaded ${successCount} contract${successCount !== 1 ? 's' : ''}`)
      } else if (failedCount > 0 && successCount === 0) {
        setError(`Failed to upload ${failedCount} contract${failedCount !== 1 ? 's' : ''}`)
      } else if (successCount > 0 && failedCount > 0) {
        setTip(`Uploaded ${successCount} contract${successCount !== 1 ? 's' : ''}, ${failedCount} failed`)
      }
    },
  ...
  const handleUploadComplete = (results: Array<{ name: string; docId: string }>) => {
    setError('');
    setTip(`Successfully uploaded ${results.length} contract${results.length !== 1 ? 's' : ''}`);
  };
  ...
  const isUploadComplete = progress.totalFiles > 0 && progress.completedFiles === progress.totalFiles
  const hasSuccessFiles = files.filter(f => f.status === 'success').length > 0
        ...
        >
          <div>
            <nav className="flex items-center space-x-2 text-sm text-gray-500 mb-3">
              <Link href="/contracts" className="hover:text-gray-700 flex items-center gap-1 transition-colors">
          ...
          <div className="bg-white rounded-xl p-4 border border-gray-200 shadow-sm">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-sm text-gray-600">AI-powered analysis in seconds</p>
              </div>
            </div>
          </div>
          <div className="bg-white rounded-xl p-4 border border-gray-200 shadow-sm">
              ...
              <div className="flex items-center justify-between">
                ...
                <div className="flex items-center gap-2">
                  ...
                  <span className={`px-3 py-1 rounded-full text-xs font-medium flex items-center gap-1.5 ${
                    apiHealthy === null ? "bg-gray-100 text-gray-600" :
                    apiHealthy ? "bg-green-100 text-green-700" : "bg-red-100 text-red-700"
                  }`}>
                    {apiHealthy ? <CheckCircle className="w-3 h-3" /> : <AlertCircle className="w-3 h-3" />}
              ...
              <div className="space-y-8">
                ...
                <div className="space-y-6">
                  <div className="flex items-center gap-2 text-sm text-gray-600">
                    ...
                    <span>Configure your upload settings (optional)</span>
                  </div>
                  ...
                  <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                    <div className="space-y-2">
                      ...
                      <input 
                        ...
                        className="w-full px-4 py-3 border border-gray-300 rounded-xl focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-all shadow-sm hover:border-gray-400" 
                      />
                      <p className="text-xs text-gray-500">The client or customer for this contract</p>
                    </div>
                    ...
                    <div className="space-y-2">
                      ...
                      <input 
                        ...
                        placeholder="e.g., Service Provider Inc" 
                        className="w-full px-4 py-3 border border-gray-300 rounded-xl focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-all shadow-sm hover:border-gray-400" 
                      />
                      <p className="text-xs text-gray-500">The supplier or vendor for this contract</p>
                    </div>
                    ...
                    <div className="space-y-2">
                      ...
                      <select 
                        ...
                        className="w-full px-4 py-3 border border-gray-300 rounded-xl focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-all shadow-sm hover:border-gray-400"
                      >
                        {packs.length === 0 && <option value="default">Default Policy Pack</option>}
                        {packs.map(p => (<option key={p.id} value={p.id}>{p.name || p.id}</option>))}
                      </select>
                      <p className="text-xs text-gray-500">Select the policy pack for analysis</p>
                    </div>
                  </div>
                    ...
                    <div className="mt-4 p-4 bg-gray-50 rounded-lg border border-gray-200">
                      ...
                      <input
                        ...
                        onChange={(e) => { 
                          ...
                          try { localStorage.setItem('x-tenant-id', v); } catch {} 
                        }}
                        placeholder="demo"
                ...
                {useModernUpload ? (
                  <div className="space-y-6">
                    <ContractUploadZone
                      onFilesAdded={handleModernFilesAdded}
                      maxFiles={15}
                      maxSize={100 * 1024 * 1024} // 100MB
                    />

                    {/* Upload Button */}
                    {files.length > 0 && !progress.isUploading && !isUploadComplete && (
                      <div className="flex justify-center">
                        <Button
                          onClick={handleModernUploadStart}
                          size="lg"
                          className="px-8 py-6 text-lg font-semibold bg-blue-600 hover:bg-blue-700 shadow-lg"
                        >
                    ...
                    {isUploadComplete && hasSuccessFiles && !hasErrorFiles && (
                      <UploadSuccessState
                        files={files}
                    ...
                    {isUploadComplete && hasErrorFiles && !hasSuccessFiles && (
                      <UploadErrorState
                        files={files}
                        onRetry={retryFile}
                        onRetryAll={() => startUpload()}
                        onRemove={removeFile}
                        onStartNew={clear}
                      />
                    )}
                ...
                <div className="flex justify-center">
                  <button
                    onClick={() => setUseModernUpload(!useModernUpload)}
                    className="text-sm text-gray-500 hover:text-gray-700 underline"
                  >
                    {useModernUpload ? 'Switch to legacy upload' : 'Switch to modern upload'}
                  </button>
                </div>
              </div>
            </CardContent>
                ...
                <div className="border-t border-gray-200 pt-4">
                  ...
                  <ul className="space-y-2">
                    <li className="flex items-center gap-2 text-sm text-gray-700">
                      ...
                      <span>Maximum file size: <strong>100MB</strong></span>
                    </li>
                    <li className="flex items-center gap-2 text-sm text-gray-700">
                      <div className="w-2 h-2 bg-blue-500 rounded-full flex-shrink-0" />
                      <span>Maximum files per upload: <strong>15</strong></span>
                    </li>
                    <li className="flex items-center gap-2 text-sm text-gray-700">
                      <div className="w-2 h-2 bg-blue-500 rounded-full flex-shrink-0" />
                  ...
                  <div className="flex gap-3">
                    ...
                    <div>
                      ...
                      <p className="text-sm text-blue-700">
                        For best results, ensure your contracts are text-based PDFs or Word documents. 
                        Scanned documents may require OCR processing which can take longer.
                      </p>
                    </div>
                  </div>
                </div>
                  ...
                  <div className="flex gap-3">
                    ...
                    <div>
                      ...
                      <p className="text-sm text-green-700">
                        Upload multiple contracts at once to save time. Our system processes up to 3 files 
                  ...
                  <div className="flex gap-3">
                    ...
                    <div>
                      ...
                      <p className="text-sm text-purple-700">
                        All uploads are encrypted in transit and at rest. Your contract data is processed 
        ...
        >
          <Card className="shadow-md border-gray-200">
            <CardHeader className="bg-gradient-to-r from-gray-50 to-white border-b border-gray-100">
              <CardTitle className="text-lg">Additional Upload Options</CardTitle>
            </CardHeader>
            <CardContent className="p-6">
              <div className="space-y-4">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  <Button 
                    ...
                    onClick={() => alert('SharePoint connector integration coming soon!')}
                  >
                    <div className="flex items-center gap-2 w-full">
                      <FileText className="w-5 h-5 text-blue-600" />
                      <span className="font-semibold">Connect to SharePoint</span>
                    </div>
                  ...
                  <Button 
                    ...
                    size="lg"
                    className="h-auto py-4 flex-col items-start gap-2 hover:border-blue-500 hover:bg-blue-50 transition-all"
                    onClick={() => alert('Bulk upload from folder coming soon!')}
                  >
                    <div className="flex items-center gap-2 w-full">
                      <UploadIcon className="w-5 h-5 text-blue-600" />
                      <span className="font-semibold">Bulk Upload from Folder</span>
                    </div>
                    <span className="text-xs text-gray-500 text-left">
                ...
                <p className="text-xs text-gray-500 text-center pt-2">
                  Additional upload methods and integrations will be available soon
                </p>
              </div>
            </CardContent>
          </Card>
