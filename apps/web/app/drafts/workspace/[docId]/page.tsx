...
type Suggestion = {
  ...
  message: string;
  createdAt: string;
  status?: "pending" | "needs-approval" | "approved" | "accepted" | "rejected";
  approverRole?: string;
};
...
function WorkspacePageClient({ params }: { params: { docId: string } }) {
  ...
  const [metaSaving, setMetaSaving] = useState<
  ...
  const [approvedHighlightIds, setApprovedHighlightIds] = useState<Set<string>>(
    new Set()
  );
  ...
  const outline = useMemo(() => {
    ...
    for (let i = 0; i < lines.length; i++) {
      ...
      if (m1) {
        items.push({ idx: i, title: `${m1[1]} ${m1[2].trim()}` });
        continue;
      }
      const m2 = L.match(reHash);
      if (m2) {
        items.push({ idx: i, title: m2[1].trim() });
        continue;
      }
    }
    ...
    if (items.length === 0) {
      ...
      for (let i = 0; i < lines.length; i++) {
        ...
        if (paraStart) {
          items.push({
            idx: i,
            title: L.trim().slice(0, 60) + (L.trim().length > 60 ? "…" : ""),
          });
          paraStart = false;
        }
      }
  ...
  const commentsBySection = useMemo(() => {
    if (comments.length === 0 || outlineWithPos.length === 0)
      return {} as Record<string, CommentItem[]>;
    ...
    for (const c of comments) {
      ...
      for (let i = 0; i < outlineWithPos.length; i++) {
        if (outlineWithPos[i].pos <= start) kIdx = i;
        else break;
      }
  ...
  const fetchOrInit = useCallback(async () => {
    try {
      ...
      let r = await fetch(`${API_BASE_URL}/api/contracts/${docId}/negotiate`, {
        headers: tenantHeaders(),
      });
      if (!r.ok) {
        // Attempt to initialize if missing
        await fetch(`${API_BASE_URL}/api/contracts/${docId}/negotiate/init`, {
          method: "POST",
          headers: tenantHeaders({ "content-type": "application/json" }),
        });
        r = await fetch(`${API_BASE_URL}/api/contracts/${docId}/negotiate`, {
          headers: tenantHeaders(),
        });
      }
      if (r.ok) {
  ...
  useEffect(() => {
    (async () => {
      try {
        const r = await fetch(`${API_BASE_URL}/policies/packs`, {
          headers: tenantHeaders(),
        });
        if (r.ok) {
    ...
    (async () => {
      try {
        const r = await fetch(`${API_BASE_URL}/api/policies/clients`, {
          headers: tenantHeaders(),
        });
        if (r.ok) {
          const j = await r.json();
          if (j && typeof j.defaults === "object")
            setClientDefaults(j.defaults);
        }
      } catch {}
    })();
  }, []);
  ...
  const loadTasks = useCallback(async () => {
    try {
      const r = await fetch(
        `${API_BASE_URL}/api/contracts/${docId}/negotiate/tasks`,
        { headers: tenantHeaders() }
      );
      if (r.ok) {
        const j = await r.json();
  ...
  const loadTemplates = useCallback(
    async (filters?: { clientId?: string; lob?: string; tags?: string[] }) => {
      try {
  ...
  const saveDraft = useCallback(async () => {
    try {
      await fetch(`${API_BASE_URL}/api/contracts/${docId}/negotiate/content`, {
        method: "POST",
        headers: tenantHeaders({ "content-type": "application/json" }),
        body: JSON.stringify({ content, by: role }),
      });
  ...
  useEffect(() => {
    ...
    const id = setTimeout(async () => {
      try {
        setAutoSave("saving");
        await fetch(
          `${API_BASE_URL}/api/contracts/${docId}/negotiate/content`,
          {
            method: "POST",
            headers: tenantHeaders({ "content-type": "application/json" }),
            body: JSON.stringify({ content, by: role }),
  ...
  const addAISuggestion = useCallback(async () => {
    try {
      const r = await fetch(
        `${API_BASE_URL}/api/contracts/${docId}/negotiate/ai/suggest`,
        { headers: tenantHeaders() }
      );
      if (!r.ok) return;
      ...
      if (!s) return;
      await fetch(`${API_BASE_URL}/api/contracts/${docId}/negotiate/suggest`, {
        method: "POST",
        headers: tenantHeaders({ "content-type": "application/json" }),
        body: JSON.stringify({
          createdBy: "ai",
  ...
  const runChecklist = useCallback(async () => {
    try {
      ...
      try {
        const g = await fetch(
          `${API_BASE_URL}/api/contracts/${docId}/negotiate`,
          { headers: tenantHeaders() }
        );
        if (g.ok) {
          const s = await g.json();
          chosenPack = s?.meta?.policyPackId;
        }
      } catch {}
      const r = await fetch(
        `${API_BASE_URL}/api/contracts/${docId}/checklist`,
        {
          method: "POST",
          headers: tenantHeaders({ "content-type": "application/json" }),
          body: JSON.stringify({
            policyPackId: chosenPack || "default",
            includeRateBaseline: true,
          }),
        }
      );
  ...
  const applyTemplate = useCallback(
    async (templateId: string) => {
      try {
        const r = await fetch(
          `${API_BASE_URL}/api/contracts/${docId}/apply-template/${templateId}`,
  ...
  const uploadTemplate = useCallback(
    async (file: File) => {
      ...
      const r = await fetch(`${API_BASE_URL}/api/templates/upload`, {
        method: "POST",
        body: fd,
        headers: tenantHeaders(),
      });
      if (r.ok) {
        await loadTemplates({
          clientId: tplClient.trim() || undefined,
          lob: tplLob.trim() || undefined,
          tags: tplTags.trim()
            ? tplTags
                .split(",")
                .map((s) => s.trim())
  ...
  const openTemplateHistory = useCallback(async (t: TemplateItem) => {
    try {
      ...
      const r = await fetch(`${API_BASE_URL}/api/templates/${t.id}/history`, {
        headers: tenantHeaders(),
      });
      if (r.ok) {
        const j = await r.json();
        const list = Array.isArray(j?.history)
          ? (j.history as TemplateItem[])
          : [];
        setTplHistory(list);
      } else {
        setTplHistory([]);
  ...
  const createNewTemplateVersion = useCallback(async () => {
    ...
    try {
      ...
      if (tplNewVersionName && tplNewVersionName.trim())
        body.name = tplNewVersionName.trim();
      if (tplNewVersionNote && tplNewVersionNote.trim())
        body.changeNote = tplNewVersionNote.trim();
      const r = await fetch(
        `${API_BASE_URL}/api/templates/${tplHistoryFor.id}/version`,
        {
          method: "POST",
          headers: tenantHeaders({ "content-type": "application/json" }),
  ...
  const acceptSuggestion = useCallback(
    async (id: string) => {
      try {
        const r = await fetch(
          `${API_BASE_URL}/api/contracts/${docId}/negotiate/suggest/${id}/resolve`,
          {
            method: "POST",
            headers: tenantHeaders({ "content-type": "application/json" }),
            body: JSON.stringify({ action: "accept", by: role }),
          }
        );
        if (r.ok) {
          await fetchOrInit();
      ...
      try {
        const r = await fetch(
          `${API_BASE_URL}/api/contracts/${docId}/negotiate/suggest/${id}/resolve`,
          {
            method: "POST",
            headers: tenantHeaders({ "content-type": "application/json" }),
            body: JSON.stringify({ action: "reject", by: role }),
          }
        );
        if (r.ok) {
          await fetchOrInit();
  ...
  const approveSuggestion = useCallback(
    async (id: string, approverRole?: string) => {
      try {
        const r = await fetch(
          `${API_BASE_URL}/api/contracts/${docId}/negotiate/suggest/${id}/approve`,
          {
            method: "POST",
            headers: tenantHeaders({ "content-type": "application/json" }),
            body: JSON.stringify({
              role: approverRole || "Procurement",
              by: role,
            }),
          }
        );
  ...
  const addComment = useCallback(async () => {
    ...
    if (!text) return;
    try {
      await fetch(`${API_BASE_URL}/api/contracts/${docId}/negotiate/comment`, {
        method: "POST",
        headers: tenantHeaders({ "content-type": "application/json" }),
        body: JSON.stringify({
          author: role,
          text,
          channel: role === "supplier" ? "supplier" : "client",
        }),
      });
      setCommentInput("");
      await fetchOrInit();
      ...
      <div className="mt-2 text-sm leading-6 border rounded p-2">
        {segs.map((s, idx) =>
          s.type === "eq" ? (
            ...
            <span
              key={idx}
              className="bg-red-100 text-red-800 line-through rounded-sm"
            >
              {s.text}
            </span>
          )
  ...
  const overlayContent = useMemo(() => {
    ...
    return (
      ...
      >
        <div
          className="p-3 font-mono text-sm leading-6 whitespace-pre-wrap break-words"
  ...
  const markerNodes = useMemo(() => {
    ...
    for (const h of highlights) {
      ...
      markers.push({
        ...
        status: h.status,
        approvedBy: (h as any).approvedBy,
        approvedAt: (h as any).approvedAt,
      });
    }
    for (const c of comments) {
      if (!c.range) continue;
  ...
  const [snapDiff, setSnapDiff] = useState<null | {
    ...
    fromVersion: number;
    toVersion: number;
    summary: { adds: number; dels: number; equals: number };
  }>(null);
  const loadSnapshots = useCallback(async () => {
    try {
      const r = await fetch(
        `${API_BASE_URL}/api/contracts/${docId}/negotiate/snapshots`,
        { headers: tenantHeaders() }
      );
      if (r.ok) {
        const j = await r.json();
  ...
  const viewSnapDiff = useCallback(
    async (sid: string) => {
      try {
        const r = await fetch(
          `${API_BASE_URL}/api/contracts/${docId}/negotiate/snapshots/${sid}/diff`,
          { headers: tenantHeaders() }
        );
        if (r.ok) setSnapDiff(await r.json());
        else setSnapDiff(null);
      } catch {
        setSnapDiff(null);
      }
    },
  ...
  // Invite helpers
  const makeInviteUrl = useCallback(
    (target: Party) => {
  ...
  const mailtoInvite = useCallback(
    (target: Party) => {
      const url = makeInviteUrl(target);
      const subject = encodeURIComponent(
        `Invitation to collaborate on contract ${docId}`
      );
  ...
  const coloredFindings = useMemo(() => {
    const list: Array<{
      kind: "policy" | "warn" | "fail" | "benchmark";
      text: string;
    }> = [];
      ...
      list.push({
        kind:
          g.status === "PASS"
            ...
            ? "warn"
            : "fail",
        text: `Governing law expected ${g.expected || "—"}, found ${
          g.found || "—"
        } (${g.status})`,
      });
    }
    if (checklist?.rateBaseline?.gaps?.length) {
      for (const gap of checklist.rateBaseline.gaps) {
        list.push({
          kind: "benchmark",
          text: `${gap.role} ${gap.seniority || ""} above P${
            checklist.rateBaseline.capPercentile
          } cap: ${gap.found.toFixed(0)} > ${gap.baseline.toFixed(
            0
          )} (Δ ${gap.delta.toFixed(0)})`,
        });
      }
    }
    if (!list.length)
      return (
        <div className="text-xs text-gray-500">
          Run checklist to see compliance and benchmark alerts.
        </div>
      );
    return (
  ...
  return (
    <div className="space-y-4">
      {toast && (
        <div
          className={`fixed right-4 top-4 z-50 px-3 py-2 rounded shadow text-sm ${
            toast.type === "success"
      ...
      <div className="flex items-center justify-between sticky top-0 bg-white/70 backdrop-blur z-30 border-b py-2 px-1">
        <div className="flex items-center gap-3">
          <Link href={`/drafts`} className="text-sm text-indigo-600">
            ← Drafts
          </Link>
          <h1 className="text-lg font-semibold flex items-center gap-2">
            <FileEdit className="w-5 h-5 text-indigo-600" /> Negotiation &
            Drafting Workspace
          </h1>
        ...
        <div className="flex items-center gap-3">
          ...
          <div className="relative">
            <button
              ...
              className="inline-flex items-center gap-1 px-3 py-1.5 rounded text-sm border hover:bg-gray-50"
            >
              ...
              <div className="absolute right-0 mt-2 w-64 rounded border bg-white shadow-md z-10 p-2 space-y-2">
                ...
                <div className="space-y-1">
                  ...
                  <div className="flex items-center justify-between">
                    ...
                    <div className="flex items-center gap-1">
                      <button
                        onClick={() => copyInvite("supplier")}
                        className="text-xs px-2 py-0.5 rounded border hover:bg-gray-50"
                      >
                        Copy
                      </button>
                      <a
                        href={mailtoInvite("supplier")}
                        className="text-xs px-2 py-0.5 rounded border"
                      >
                        Email
                      </a>
                ...
                <div className="text-[11px] text-gray-500">
                  Links include role preselection. Add auth or tokens later.
                </div>
              </div>
            )}
          </div>
          ...
          <div className="hidden lg:flex items-center gap-2 px-2">
            ...
            <div className="flex items-center gap-2">
              ...
              <input
                ...
                placeholder="ID"
                className="border rounded px-2 py-1 text-sm w-28 hover:bg-gray-50"
              />
            </div>
            <div className="flex items-center gap-2">
              <div className="flex items-center gap-1 text-xs text-gray-600">
                <Briefcase className="w-3.5 h-3.5" /> Supplier
              </div>
              <input
                value={supplierId}
            ...
            <div className="flex items-center gap-2">
              <div className="flex items-center gap-1 text-xs text-gray-600">
                <Settings2 className="w-3.5 h-3.5" /> Pack
              </div>
            ...
            <button
              onClick={async () => {
                try {
                  ...
                  const r = await fetch(
                    `${API_BASE_URL}/api/contracts/${docId}/negotiate/meta`,
                    {
                      method: "POST",
                      headers: tenantHeaders({
              ...
              disabled={
                metaSaving === "saving" || (Boolean(clientId) && !policyPackId)
              }
              className={`inline-flex items-center gap-1 px-3 py-1.5 rounded text-sm border hover:bg-gray-50 ${
                metaSaving === "saving" || (clientId && !policyPackId)
            ...
            >
              {metaSaving === "saving" ? (
                ...
                <>
                  <Save className="w-4 h-4" /> Save meta
                </>
              )}
            </button>
          </div>
          ...
          <select
            value={role}
            onChange={(e) => setRole(e.target.value as Party)}
            className="border rounded px-2 py-1 text-sm hover:bg-gray-50"
          >
          ...
          <button
            onClick={saveDraft}
            className="inline-flex items-center gap-1 px-3 py-1.5 rounded text-sm bg-indigo-600 text-white hover:bg-indigo-700"
          >
            <Save className="w-4 h-4" /> Save
          </button>
          <a
            href={`${API_BASE_URL}/api/contracts/${docId}/negotiate/export`}
            target="_blank"
            className="inline-flex items-center gap-1 px-3 py-1.5 rounded text-sm border hover:bg-gray-50"
          >
      ...
      <div className="grid grid-cols-1 lg:grid-cols-[220px,1fr,360px] gap-4">
        ...
        <div className="rounded border p-3 space-y-2">
          ...
          <ul className="text-sm space-y-1 max-h-72 overflow-auto">
            {outlineWithPos.length === 0 ? (
              <li className="text-xs text-gray-500">No sections detected</li>
            ) : (
              outlineWithPos.map((o, i) => (
                <li
                  key={`${o.idx}-${i}`}
                  className="truncate cursor-pointer hover:underline"
                  onClick={() => jumpToPos(o.pos)}
                >
                  {o.title}
                </li>
              ))
        ...
        <div className="space-y-2">
            ...
            <div className="flex items-center gap-2">
              <button
                ...
                className={`flex items-center gap-1 text-xs px-2 py-1 rounded border ${
                  trackChanges ? "bg-indigo-50 border-indigo-300" : ""
                }`}
              >
                <Highlighter className="w-3.5 h-3.5" /> Track changes
              </button>
              <button
                title="Toggle highlights"
                onClick={() => setShowHighlights((v) => !v)}
                className={`flex items-center gap-1 text-xs px-2 py-1 rounded border ${
                  showHighlights ? "bg-yellow-50 border-yellow-300" : ""
                }`}
              >
              ...
              <button
                ...
                className="flex items-center gap-1 text-xs px-2 py-1 rounded bg-emerald-600 text-white"
              >
              ...
              <button
                ...
                className="flex items-center gap-1 text-xs px-2 py-1 rounded border"
              >
                <ShieldCheck className="w-3.5 h-3.5" /> Checklist
              </button>
              <div className="hidden md:flex items-center gap-1 ml-2">
                ...
                <button
                  ...
                  className={`text-[11px] px-2 py-0.5 rounded border ${
                    !searchMatches.length ? "opacity-50 cursor-not-allowed" : ""
                  }`}
                >
                  Prev
                </button>
                ...
                {tplOpen && (
                  <div className="absolute z-10 mt-2 w-80 rounded border bg-white shadow p-2">
                    <div className="text-xs text-gray-600 mb-1">
                      Apply a template or upload a new one
                    </div>
                    <div className="grid grid-cols-3 gap-1 mb-2">
                      <input
                        value={tplClient}
                        onChange={(e) => setTplClient(e.target.value)}
                        placeholder="Client"
                        className="border rounded px-1 py-0.5 text-xs col-span-1"
                      />
                      <input
                        value={tplLob}
                        onChange={(e) => setTplLob(e.target.value)}
                        placeholder="LOB"
                        className="border rounded px-1 py-0.5 text-xs col-span-1"
                      />
                      <input
                      ...
                      <div className="col-span-3 flex items-center justify-end gap-1">
                        ...
                        <button
                          ...
                          className="text-[11px] px-2 py-0.5 rounded bg-indigo-600 text-white"
                        >
                          Filter
                        </button>
                      </div>
                                ...
                                {t.tags?.length ? (
                                  <div className="mt-0.5 flex flex-wrap gap-1">
                                    {t.tags.map((tag) => (
                                      <span
                                        key={tag}
                                        className="text-[10px] px-1 py-0.5 rounded bg-gray-100"
                                      >
                                        {tag}
                                      </span>
                                    ))}
                  ...
                  <div className="relative z-50 w-[680px] max-w-[95vw] bg-white rounded shadow border p-3">
                    ...
                    <div className="mt-2 grid grid-cols-2 gap-2">
                      ...
                      <div className="border rounded p-2">
                        ...
                        <div className="space-y-2">
                          <input
                            value={tplNewVersionName}
                            onChange={(e) =>
                              setTplNewVersionName(e.target.value)
                            }
                            placeholder="Name (optional)"
                            className="w-full border rounded px-2 py-1 text-sm"
                          />
                          <input
                            value={tplNewVersionNote}
                            onChange={(e) =>
                              setTplNewVersionNote(e.target.value)
                            }
                          ...
                          <div className="flex items-center justify-end gap-2">
                            ...
                            <button
                              onClick={createNewTemplateVersion}
          ...
          <div className="relative">
            ...
            <div
              className="absolute top-0 right-1 h-full w-6"
            ...
            >
              {markerNodes.map((m) => (
                <div
                  ...
                  style={{
                    top: Math.max(4, m.top),
                    background:
                      m.type === "highlight" ? m.color || "#fef08a" : "#fff",
                  }}
                  onMouseEnter={() =>
                    setHoverMarker({
                      id: m.id,
                      ...
                      approvedBy: (m as any).approvedBy,
                      approvedAt: (m as any).approvedAt,
                    })
                  }
            ...
            {hoverMarker && (
              <div
                className="absolute right-8 z-20 max-w-xs bg-white text-gray-900 text-xs rounded border shadow p-2 space-y-2"
                style={{ top: Math.max(8, hoverMarker.top) }}
              >
                <div className="text-[11px] text-gray-600">
                  {hoverMarker.type === "highlight" ? "Highlight" : "Comment"}
                </div>
                <div className="text-xs">
                  {hoverMarker.text ||
                    (hoverMarker.type === "highlight" ? "No note" : "")}
                </div>
                {hoverMarker.type === "highlight" &&
                  hoverMarker.status === "approved" && (
                    <div className="text-[11px] text-emerald-700">
                      Approved by {hoverMarker.approvedBy || "—"}{" "}
                      {hoverMarker.approvedAt
                        ? `· ${new Date(
                            hoverMarker.approvedAt
                          ).toLocaleString()}`
                        : ""}
                  ...
                  {hoverMarker.type === "highlight" && (
                    <button
                      ...
                      onClick={async () => {
                        try {
                          await fetch(
                            `${API_BASE_URL}/api/contracts/${docId}/negotiate/highlight/${hoverMarker.id}/approve`,
                            {
                              method: "POST",
                              headers: tenantHeaders({
                                "content-type": "application/json",
                          ...
                          setApprovedHighlightIds((prev) =>
                            new Set(prev).add(hoverMarker.id)
                          );
                          await fetchOrInit();
                        } finally {
                          setHoverMarker(null);
                ...
                <div className="flex items-center gap-1">
                  ...
                  <button
                    ...
                    onClick={async () => {
                      const m = markerNodes.find(
                        (mm) => mm.id === hoverMarker.id
                      );
                      if (!m) return;
                      try {
                        await fetch(
                          `${API_BASE_URL}/api/contracts/${docId}/negotiate/comment`,
                          {
                            method: "POST",
                            headers: tenantHeaders({
                              "content-type": "application/json",
            ...
            <textarea
              ...
              onChange={(e) => {
                ...
                while (
                  s < oldText.length - p &&
                  s < newText.length - p &&
                  oldText[oldText.length - 1 - s] ===
                    newText[newText.length - 1 - s]
                )
                  s++;
                ...
                if (delta !== 0) {
                  ...
                  const adj = (r?: Range): Range | undefined => {
                    ...
                    if (at > r.start && at <= r.end)
                      return {
                        start: r.start,
                        end: Math.max(r.start, r.end + delta),
                      };
                    return r;
                  };
                  setHighlights((hs) =>
                    hs.map((h) => (h.range ? { ...h, range: adj(h.range) } : h))
                  );
                  setComments((cs) =>
                    cs.map((c) =>
                      c.range ? ({ ...c, range: adj(c.range) } as any) : c
                    )
                  );
                }
                setContent(newText);
            ...
            {selOpen && selection && (
              <div className="absolute right-2 top-2 z-10 bg-white border rounded shadow p-2 text-xs flex flex-col gap-1 w-72">
                ...
                <div className="grid grid-cols-3 gap-1">
                  <input
                    value={selApprover}
                    onChange={(e) => setSelApprover(e.target.value)}
                    placeholder="Approver role"
                    className="border rounded px-1 py-0.5 col-span-3"
                  />
                  <input
                    value={selNote}
                  ...
                  />
                  <input
                    value={selProposedText}
                    onChange={(e) => setSelProposedText(e.target.value)}
                    placeholder="Propose replacement (optional)"
                    className="border rounded px-1 py-0.5 col-span-3"
                  />
                </div>
                <div className="flex items-center justify-end gap-2">
                  <button
                    onClick={async () => {
                      try {
                        ...
                        const derivedColor =
                          selApprover.trim().toLowerCase() === "procurement"
                            ? "#fef08a"
                            : "#dbeafe";
                        await fetch(
                          `${API_BASE_URL}/api/contracts/${docId}/negotiate/highlight`,
                          {
                            method: "POST",
                            headers: tenantHeaders({
                              "content-type": "application/json",
                        ...
                        if (selProposedText.trim()) {
                          await fetch(
                            `${API_BASE_URL}/api/contracts/${docId}/negotiate/suggest`,
                            {
                              method: "POST",
                              headers: tenantHeaders({
                                "content-type": "application/json",
                              ...
                              body: JSON.stringify({
                                createdBy: role,
                                type: "change",
                                message: selNote || "Proposed change",
                                approverRole: selApprover.trim() || undefined,
                                patch: {
                                  position: {
                                    start: selection.start,
                                    end: selection.end,
                                  },
                                  to: selProposedText.trim(),
                                },
                              }),
                            }
                          );
                          ...
                          await fetch(
                            `${API_BASE_URL}/api/contracts/${docId}/negotiate/suggest`,
                            {
                              ...
                              body: JSON.stringify({
                                createdBy: role,
                                type: "policy",
                                message: selNote || `Approval required`,
                                range: selection,
                                approverRole: selApprover.trim(),
                              }),
                            }
                          );
                        }
                        ...
                        {list
                          ...
                          .map((c) => (
                            <div
                              ...
                              style={{
                                background:
                                  c.author === "client" ? "#dbeafe" : "#ffedd5",
                              }}
                              onClick={() =>
                                jumpToPos((c as any).range?.start ?? 0)
                              }
                            >
                              <div className="flex items-center justify-between">
                                <div className="text-[11px] text-gray-600">
                                ...
                                {c.status !== "resolved" && (
                                  <button
                                    ...
                                    onClick={async (e) => {
                                      e.stopPropagation();
                                      await fetch(
                                        `${API_BASE_URL}/api/contracts/${docId}/negotiate/comment/${c.id}/status`,
                                        {
                                          method: "POST",
                                          headers: tenantHeaders({
                                            "content-type": "application/json",
          ...
          <div className="flex items-center gap-2 text-sm border-b pb-2">
            <button
              ...
              className={`px-2 py-1 rounded ${
                activeRightTab === "ai" ? "bg-indigo-600 text-white" : "border"
              }`}
            >
              <Sparkles className="inline w-4 h-4 mr-1" /> AI
            </button>
            <button
            ...
            >
              <ShieldCheck className="inline w-4 h-4 mr-1" /> Compliance
            </button>
            <button
            ...
            >
              <Workflow className="inline w-4 h-4 mr-1" /> Tasks
            </button>
          </div>
            ...
            {activeRightTab === "ai" && (
              <div className="space-y-2">
                ...
                {aiSuggestions.length === 0 ? (
                  <div className="text-xs text-gray-500">
                    No suggestions yet. Click AI Suggest to add one.
                  </div>
                ) : (
                  <div className="space-y-2">
                        ...
                        <div className="mt-1 flex items-center gap-2">
                          ...
                          {s.status === "needs-approval" && (
                            <>
                              <button
                                onClick={() =>
                                  approveSuggestion(s.id, s.approverRole)
                                }
              ...
              <div className="space-y-2">
                ...
                {approvalTasks.length === 0 ? (
                  <div className="rounded border p-2 flex items-center gap-2 text-xs">
                    <CheckCheck className="w-4 h-4 text-emerald-600" /> No
                    pending approvals.
                  </div>
                ) : (
                  <div className="space-y-2">
                    {approvalTasks.map((t) => (
                      <div key={t.id} className="rounded border p-2 text-sm">
                        <div className="text-[11px] text-gray-500 mb-1">
                          Needs approval
                          {t.approverRole ? ` · ${t.approverRole}` : ""}
                        </div>
                        <div>{t.message}</div>
                        <div className="mt-1 flex items-center gap-2">
                          <button
                            onClick={() =>
                              approveSuggestion(t.id, t.approverRole)
                            }
                            className="text-xs px-2 py-1 rounded bg-amber-600 text-white"
                          >
                            Approve
        ...
        <div className="fixed inset-0 z-40 flex items-center justify-center">
          ...
          <div className="relative z-50 w-[680px] max-w-[95vw] bg-white rounded shadow border p-3">
            ...
            <div className="mt-2 flex items-center justify-between">
              <div className="text-xs text-gray-600">
                Create a snapshot to mark a version and compare later.
              </div>
              <button
                onClick={createSnap}
                className="text-xs px-2 py-1 rounded border"
            ...
            <div className="mt-2 grid grid-cols-2 gap-2">
                    ...
                    snapshots
                      ...
                      .map((s) => (
                        ...
                        >
                          <div className="min-w-0">
                            <div className="text-sm truncate">
                              {new Date(s.createdAt).toLocaleString()} · v
                              {s.version}
                            </div>
                            <div className="text-[11px] text-gray-500 truncate">
                              By {s.author}
              ...
              <div className="border rounded p-2">
                ...
                {!snapDiff ? (
                  <div className="p-2 text-xs text-gray-500">
                    Select a snapshot to compare with current.
                  </div>
                ) : (
                  <div className="text-sm space-y-1">
                    <div>
                      From v{snapDiff.fromVersion} → To v{snapDiff.toVersion}
                    </div>
                    <div className="text-[11px] text-gray-600">
                      Adds: {snapDiff.summary.adds} · Deletes:{" "}
                      {snapDiff.summary.dels} · Unchanged:{" "}
                      {snapDiff.summary.equals}
                    </div>
                  </div>
                )}
              </div>
