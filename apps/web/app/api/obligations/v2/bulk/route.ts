...
export const POST = withAuthApiHandler(async (request: NextRequest, ctx) => {
  ...
  if (!obligationIds || !Array.isArray(obligationIds) || obligationIds.length === 0) {
    return createErrorResponse(ctx, 'BAD_REQUEST', 'obligationIds array is required', 400);
  }
  ...
  if (obligations.length !== obligationIds.length) {
    return createErrorResponse(ctx, 'BAD_REQUEST', 'Some obligations not found or unauthorized', 400);
  }

  switch (action) {
    case 'update_status': {
      if (!data?.status) {
        return createErrorResponse(ctx, 'BAD_REQUEST', 'data.status is required', 400);
      }
    ...
    case 'assign': {
      if (!data?.assignedToUserId) {
        return createErrorResponse(ctx, 'BAD_REQUEST', 'data.assignedToUserId is required', 400);
      }
    ...
    case 'reschedule': {
      if (!data?.dueDate) {
        return createErrorResponse(ctx, 'BAD_REQUEST', 'data.dueDate is required', 400);
      }
    ...
    case 'add_tags': {
      if (!data?.tags || !Array.isArray(data.tags)) {
        return createErrorResponse(ctx, 'BAD_REQUEST', 'data.tags array is required', 400);
      }
    ...
    default:
      return createErrorResponse(ctx, 'BAD_REQUEST', 'Invalid action. Use: update_status, assign, reschedule, add_tags, delete', 400);
  }
});
