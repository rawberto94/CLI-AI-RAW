...
export const GET = withAuthApiHandler(async (request: NextRequest, ctx) => {
  ...
  // Services available for future enhanced orchestration
  ...
  switch (action) {
    ...
    default:
      return createErrorResponse(ctx, 'BAD_REQUEST', 'Invalid action. Use: status, triggers, or analytics', 400);
  }
});
  ...
  switch (action) {
    case 'create_goal': {
      ...
      if (!goalType || !description) {
        return createErrorResponse(ctx, 'BAD_REQUEST', 'Goal type and description are required', 400);
      }
