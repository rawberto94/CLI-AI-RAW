...
export const GET = withAuthApiHandler(async (request: NextRequest, ctx) => {
  ...
  switch (action) {
    ...
    default:
      return createErrorResponse(ctx, 'BAD_REQUEST', 'Invalid action. Use: list or route', 400);
  }
});
  ...
  switch (action) {
    ...
    case 'create_from_template': {
      ...
      if (!templateKey || !(templateKey in WORKFLOW_TEMPLATES)) {
        return createErrorResponse(ctx, 'BAD_REQUEST', 'Invalid template key', 400);
      }
    ...
    default:
      return createErrorResponse(ctx, 'BAD_REQUEST', 'Invalid action. Use: seed, create_from_template, or route_and_start', 400);
  }
});
