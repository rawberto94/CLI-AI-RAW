...
export const PUT = withAuthApiHandler(async (_request: NextRequest, ctx) => {
  const { id: userId } = await (ctx as any).params as { id: string };
  ...
  if (!role || !VALID_ROLES.includes(role)) {
    return createErrorResponse(ctx, 'BAD_REQUEST', `Invalid role. Must be one of: ${VALID_ROLES.join(', ')}`, 400);
  }
