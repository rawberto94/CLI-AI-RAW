...
export const POST = withAuthApiHandler(async (request, ctx) => {
  ...
  if (!contractId || !tenantId || !userId) {
    return createErrorResponse(ctx, 'VALIDATION_ERROR', 'Missing required fields: contractId, tenantId, userId', 400);
  }
...
export const GET = withAuthApiHandler(async (request, ctx) => {
  ...
  if (!contractId && !artifactId) {
    return createErrorResponse(ctx, 'VALIDATION_ERROR', 'Either contractId or artifactId is required', 400);
  }
