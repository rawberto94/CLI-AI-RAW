...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    if (process.env.NODE_ENV === "production" && process.env.SMTP_HOST) {
      logger.info(`[Auth] Verification email queued for: ${user.email}`);
    } else {
      logger.info('EMAIL VERIFICATION LINK (dev mode)', { action: 'email-verification', email: user.email, verifyUrl, expiresAt: expiresAt.toISOString() });
    }
