...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    // To prevent a timing-oracle on user-enumeration (the "user exists" branch
    // used to perform DB writes + an SMTP round-trip synchronously before
    // responding, while the "no user" branch returned instantly — response
    // latency revealed which emails had accounts), the heavy work runs
    // fire-and-forget. The handler returns the same message on both branches
    ...
    void (async () => {
      try {
        ...
        logger.error("[Auth] Forgot-password background pipeline failed", bgError);
      }
    })();
