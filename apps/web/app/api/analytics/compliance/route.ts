  ...
  try {
    ...
    switch (action) {
      ...
      case 'recommend-remediation':
        if (!complianceResult) {
          return NextResponse.json({ error: 'Compliance result required' }, { status: 400 });
        }
        const remediationPlan = await complianceEngine.recommendRemediation(complianceResult);
