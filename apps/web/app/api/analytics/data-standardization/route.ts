...
import { dataStandardizationService } from "@/lib/data-orchestration";
  ...
  try {
    ...
    switch (action) {
      ...
      case "cluster_line_of_service":
        result = await dataStandardizationService.clusterLineOfService(
          tenantId
        );
        break;
      ...
      case "standardize_value":
        ...
        if (!value || !category) {
          return NextResponse.json(
            {
              success: false,
              error: "Value and category parameters are required",
            },
            { status: 400 }
          );
        }
        ...
        switch (category) {
          case "line_of_service":
            result = await dataStandardizationService.standardizeLineOfService(
              value
            );
            break;
          case "supplier":
            result = await dataStandardizationService.standardizeSupplier(
              value
            );
            break;
          case "role":
          ...
          case "seniority":
            ...
            result = await dataStandardizationService.standardizeSeniority(
              value,
              {
          ...
          switch (category) {
            case "line_of_service":
              standardizationResult =
                await dataStandardizationService.standardizeLineOfService(
                  value
                );
              break;
            case "supplier":
              standardizationResult =
                await dataStandardizationService.standardizeSupplier(value);
              break;
            case "role":
              standardizationResult =
                await dataStandardizationService.standardizeRole(value);
              break;
            case "seniority":
              standardizationResult =
                await dataStandardizationService.standardizeSeniority(value);
              break;
            default:
              standardizationResult = {
                originalValue: value,
        ...
        result = {
          ...
          summary: {
            averageConfidence:
              ...
              batchResults.length,
            highConfidenceCount: batchResults.filter((r) => r.confidence > 0.8)
              .length,
            uniqueStandardValues: new Set(
              batchResults.map((r) => r.standardValue)
            ).size,
          },
        };
        break;
      ...
      case "create_standardization_rule":
        ...
        if (!sourceValue || !standardValue || !ruleCategory) {
          return NextResponse.json(
            {
              success: false,
              error: "Source value, standard value, and category are required",
            },
            { status: 400 }
          );
        }
