...
export async function GET(request: NextRequest) {
  try {
    ...
    if (includeAnalytics && benchmarks.length > 0) {
      const avgRate =
        benchmarks.reduce((sum, b) => sum + b.avgRate, 0) / benchmarks.length;
      ...
      response.analytics = {
        ...
        performanceMetrics: {
          avgPerformanceScore: Math.round(
            benchmarks.reduce((sum, b) => sum + b.performanceScore, 0) /
              benchmarks.length
          ),
    ...
    const mockBenchmarkResults = {
      ...
      marketInsights: {
        ...
        outliers: [
          ...
          {
            supplier: "Budget Solutions",
            rate: 95,
            reason: "Significantly below market",
          },
        ],
      },
